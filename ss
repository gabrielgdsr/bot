var Service = Class.extend({init:function ()
{
    this.transactionCallback = this.errorCallback = this.successCallback = null;
...
{
    this.successCallback = this.errorCallback = null;
    null != this.transactionCallback && this.transactionCallback();
    this.transactionCallback = null
}}), DurationMismatchService = Service.extend({init:function ()
{
    this._super()
}, execute:function ()
{
...
}}), DurationUpdateService = Service.extend({init:function (a, b, c)
{
    this._super();
    this.auth = a;
    this.cid = b;
...
{
    rpcGW.execute("duration.update",
        this, this.auth, this.cid, this.duration)
}, parse:function (a)
{
    this.duration = this.cid = this.auth = null;
    if (0 != a.status)this.onStatusError(a.status)
}}), HistorySelectService = Service.extend({init:function ()
{
    this._super()
}, execute:function ()
{
...
}}), APIDispatcher = EventDispatcher.extend({init:function ()
{
    this._super();
    this.CHAT = "chat";
...
}, getUsers:function ()
{
    ...
    }, waitListLeave:function ()
    {
        if (this.isReady)try
        {
            if (!Models.room.userInBooth &&
                Models.room.data.waitListEnabled && null != Models.room.getWaitListPosition())Room.onWaitListLeaveConfirm()
        } catch (a)
        {
            console.error(a)
        }
    ...
    }, moderateSetRole:function (a, b)
    {
        if (this.isReady)try
        {
            a && !isNaN(b) && -1 < b && 5 > b && (Models.room.admins[Models.user.data.id] ||
                Models.room.ambassadors[Models.user.data.id] && !Models.room.data.staff[a] || Models.user.hasPermission(Models.user.MANAGER) && (!Models.room.data.staff[a] || Models.room.data.staff[a] < Models.room.data.staff[Models.user.data.id])) && new ModerationPermissionsService(a, b)
        } catch (c)
        {
            console.error(c)
        }
    ...
    }}), GAETransactionManager = Class.extend({init:function ()
{
    this.queue = [];
    this.startTime = this.timeoutID =
        this.count = 0
}, reset:function ()
{
...
}, onTransactionComplete:function ()
{
    ...
    this.queue.shift();
    0 < this.queue.length && (this.timeoutID = setTimeout($.proxy(this.onTimeoutComplete, this), 10))
}}), WindowMessageListener = Class.extend({init:function ()
{
...
{
    if (a.origin === this.ytOrigin)if ("playbackReady" == a.data)this.ytSource = a.source, this.ytOrigin =
        a.origin, Playback.player = !0; else if (0 == a.data.indexOf("onPlayerError"))Playback.onYTPlayerError(a.data.split("=").pop()); else"playbackComplete" == a.data ? Playback.playbackComplete() : 0 == a.data.indexOf("durationCheck") && Playback.durationCheck(parseInt(a.data.split("=").pop()))
}, ytx:function (a)
{
    this.ytSource && this.ytSource.postMessage(a, this.ytOrigin)
}}), socket, Socket = Class.extend({init:function (a)
{
    this.url = a;
...
}, open:function ()
{
    ...
    a = Slug ? new RoomJoinService(Slug) : new RoomStateService;
...
{
    a && a.wasClean ? alert("This session has now ended. Goodbye.") : (console.info("closing", a), a = this.backoff, 5 >= a && (a +=
        1), socketInit(a))
}, execute:function (a, b)
{
    var c = Array.prototype.slice.call(arguments).slice(2), d = this._rpc_id++;
    this.rpcs[d] = b;
    this.send({type:"rpc", id:d, name:a, args:c})
}, chat:function (a)
{
    this.send({type:"chat", msg:a})
}, message:function (a)
{
    a = JSON.parse(a.data);
    if (a.type && "rpc" === a.type)
    {
        ...
        {
            var c = this.rpcs[b];
            if (c)if (delete this.rpcs[b], 999 === a.status)this.socketLog(a), c.onFault(a.result); else c.onResult({status:a.status, data:a.result}); else this.socketLog(a, "no responder")
        } else this.socketLog(a,
            "no rpc id")
    } else if (Models.room.joined)try
    {
        if (a.room && a.room != Models.room.data.id)this.socketLog(a, "room mismatch"); else if (b = a.messages)
        {
            c = b.length;
            for (a = 0; a < c; ++a)
            {
                var d = b[a].type, e = b[a].data, f = SocketListener[d];
                this.socketLog(b[a]);
                if (f)try
                {
                    f(e)
...
{
    Models.room.admins[Models.user.data.id] && (b && console.log(b),
        a && console.log(a))
}});
...
var SocketMessageListener = Class.extend({init:function ()
    {
    }, chat:function (a)
    {
    ...
    {
        Models.chat.receive({type:"skip", message:Lang.messages.djSkip.split("%NAME%").join($("<span/>").text(a.username).html())});
    ...
    {
        var b;
        Models.user.data.id == a.id ? new RoomJoinService(Models.room.data.id) : (Models.room.userHash[a.id] &&
            (b = Utils.clone(Models.room.userHash[a.id])), Models.room.userLeave(a.id), b && API.delayDispatch(API.USER_LEAVE, b))
    ...
    {
        Models.chat.receive({type:"update", message:Lang.messages.follow.split("%NAME%").join($("<span/>").text(a.un).html()), language:Models.user.data.language});
        Models.user.addFollower(a.id);
        API.delayDispatch(API.USER_FAN, Models.room.userHash[a.id])
    }, followJoin:function (a)
    {
        1 <
            a.r ? (Models.chat.receive({type:"update", message:Lang.messages.friendEnter.split("%NAME%").join($("<span/>").text(a.un).html()), language:Models.user.data.language}), 3 == a.r ? Models.user.addFriend(a.id) : Models.user.addFollowing(a.id), API.delayDispatch(API.FRIEND_JOIN, Models.room.userHash[a.id])) : (Models.chat.receive({type:"update", message:Lang.messages.fanEnter.split("%NAME%").join($("<span/>").text(a.un).html()), language:Models.user.data.language}), Models.user.addFollower(a.id), API.delayDispatch(API.FAN_JOIN,
            Models.room.userHash[a.id]))
    }, voteUpdate:function (a)
    {
        Models.room.voteUpdate(a);
        var b = Models.room.userHash[a.id];
        b && API.delayDispatch(API.VOTE_UPDATE, {user:b, vote:a.vote})
    }, voteUpdateMulti:function (a)
    {
        for (var b in a.votes)
        {
            Models.room.voteUpdate({id:b, vote:a.votes[b]});
            var c = Models.room.userHash[b];
            c && API.delayDispatch(API.VOTE_UPDATE, {user:c, vote:a.votes[b]})
        }
    }, curateUpdate:function (a)
    {
        Models.room.curateUpdate(a);
        (a = Models.room.userHash[a.id]) && API.delayDispatch(API.CURATE_UPDATE, {user:a})
    ...
    {
        Models.chat.receive({type:"skip",
            message:Lang.messages.roomSkip});
        API.delayDispatch(API.VOTE_SKIP, null)
    }, modSkip:function (a)
    {
        Models.chat.receive({type:"moderation", from:a.moderator, message:Lang.messages.modSkip, language:Models.user.data.language});
        API.delayDispatch(API.MOD_SKIP, a.moderator)
    }, modKick:function (a)
    {
        Models.chat.receive({type:"moderation", from:a.moderator, message:Lang.messages.modKick.split("%NAME%").join($("<span/>").text(a.username).html()), language:Models.user.data.language})
    }, modBan:function (a)
    {
        Models.chat.receive({type:"moderation",
            from:a.moderator, message:Lang.messages.modBan.split("%NAME%").join($("<span/>").text(a.username).html()), language:Models.user.data.language})
    }, modAddDJ:function (a)
    {
        Models.chat.receive({type:"moderation", from:a.moderator, message:Lang.messages.modAddDJ.split("%NAME%").join($("<span/>").text(a.username).html()), language:Models.user.data.language})
    }, modRemoveDJ:function (a)
    {
        Models.chat.receive({type:"moderation", from:a.moderator, message:Lang.messages.modRemoveDJ.split("%NAME%").join($("<span/>").text(a.username).html()),
            language:Models.user.data.language})
    }, modAddWaitList:function (a)
    {
        Models.chat.receive({type:"moderation", from:a.moderator, message:Lang.messages.modAddWaitList.split("%NAME%").join($("<span/>").text(a.username).html()), language:Models.user.data.language})
    }, modRemoveWaitList:function (a)
    {
        Models.chat.receive({type:"moderation", from:a.moderator, message:Lang.messages.modRemoveWaitList.split("%NAME%").join($("<span/>").text(a.username).html()), language:Models.user.data.language})
    }, modStaff:function (a)
    {
        var b,
            c;
        for (c in a.users)if (Models.room.userHash[c] && (0 == a.users[c] ? b = Lang.messages.staffNone : 1 == a.users[c] ? b = Lang.messages.staffFeaturedDJ : 2 == a.users[c] ? b = Lang.messages.staffBouncer : 3 == a.users[c] ? b = Lang.messages.staffManager : 4 == a.users[c] ? b = Lang.messages.staffCohost : 5 == a.users[c] && (b = Lang.messages.staffHost), b))Models.chat.receive({type:"moderation", from:a.moderator, message:b.split("%NAME%").join($("<span/>").text(Models.room.userHash[c].username).html()), language:Models.user.data.language}), Models.room.editStaff(c,
            a.users[c])
    ...
    {
        Playback.media &&
            Playback.media.cid == a.cid.split(":").pop() ? 0 < ytDuration ? new DurationUpdateService(a.auth, a.cid, ytDuration) : (this.durationAuth = a.auth, this.durationCID = a.cid, this.durationTimeoutID = setTimeout($.proxy(this.requestDurationRetry, this), 1E3)) : new DurationUpdateService(a.auth, a.cid, 0)
    ...
    {
        Playback.durationUpdate(a.cid,
            a.duration);
        LS.media.updateDuration(a.cid, a.duration)
    }, ping:function ()
    {
        Models.room.admins[Models.user.data.id] && DB.settings && DB.settings.showPings && Models.chat.receive({type:"system", message:"Server: Ping Received " + Utils.getSimpleTimestamp(), language:Models.user.data.language});
        socket.execute("user.pong", null)
    }, pdjUpdate:function ()
    {
        Playback.play();
        Playback.streamDisabled = !0;
        Dialog.alert(Lang.alerts.updateMessage, $.proxy(Utils.forceRefresh, Utils), Lang.alerts.update, !0)
    }, pdjMessage:function (a)
    {
        Models.chat.receive({type:"system",
            message:a, language:Models.user.data.language})
    }, lotteryWinner:function (a)
    {
        for (var b = a.length, c = 0; c < b; ++c)
        {
            var d = Models.room.userHash[a[c]];
            d && Models.chat.receive({type:"update", message:Lang.messages.lotteryWinner.split("%USER%").join($("<span/>").text(d.username).html()), language:Models.user.data.language})
        }
    }, kick:function (a)
    {
        window.location.href = "/?ref=" + a.ref + "&reason=" + encodeURIComponent(a.reason) + "&who=" + encodeURIComponent(a.moderator)
    }, chatDelete:function (a)
    {
        a.username = a.moderator;
        Models.chat.onChatDelete(a)
    }}),
    LSDatabase = Class.extend({init:function ()
    {
    ...
    {
        var a = $.browser.mozilla || $.browser.msie ? 200 : 150, a = localStorage.getItem("settings") ? JSON.parse(localStorage.getItem("settings")) : {volume:50, chatSound:"mentions",
            chatTranslation:!0, showPings:!0, avatarcap:a, streamDisabled:!1, chatTS:12};
        localStorage.clear();
    ...
    }}), LSLocalStore = Class.extend({init:function ()
    {
    ...
    }, getMediaPlaylists:function (a)
    {
        ...
        for (c in DB.playlist)for (var d in DB.playlist[c].items)
        {
            ...
            e && e.cid == a && b.push({id:DB.playlist[c].id,
                name:DB.playlist[c].name})
        }
        return b
    }}), LSMediaStore = Class.extend({init:function ()
    {
    ...
    }, write:function (a)
    {
        if (a && 0 < a.length)
        {
            ...
            LS.lastMod.write({name:"media", modified:a[a.length - 1].modified});
    ...
    }}), LSLastModStore = Class.extend({init:function ()
    {
    }, write:function (a)
    {
        DB.lastModTime[a.name] =
            a
    }, read:function (a)
    {
        return(a = DB.lastModTime[a]) ? a : {name:null, modified:"0,0000000000"}
    }, remove:function (a)
    {
        delete DB.lastModTime[a]
    }}), LSPlaylistStore = Class.extend({init:function ()
    {
    ...
    }, write:function (a)
    {
        ...
        if (a && 0 < a.length)
        {
            ...
            LS.lastMod.write({name:"playlists", modified:a[a.length - 1].modified});
    ...
    }, exists:function (a)
    {
        ...
        }, checkPermaDelete:function (a)
        {
            ...
            b || LS.media.remove(a);
            return!b
        }}), LSPlaylistMediaStoreRead = Class.extend({init:function (a)
    {
        this.id = a;
    ...
    {
        for (var b = DB.playlist[this.id].items, c = b.length, d = 0, e = 0; e < c; ++e)
        {
            var f = DB.media[b[e]];
        ...
        this.lookupCallback &&
        this.lookupCallback(d)
    }}), LSPlaylistStoreRead = Class.extend({init:function ()
    {
    ...
    }}), LSMediaSearchStoreRead = Class.extend({load:function (a)
    {
        var b = [], c;
        for (c in DB.playlist)for (var d = DB.playlist[c].items, e = d.length, f = 0; f < e; ++f)
        {
            var g = DB.media[d[f]];
            if (g && g.author &&
                g.title && (-1 < g.author.toLowerCase().indexOf(a) || -1 < g.title.toLowerCase().indexOf(a)))g = Utils.clone(g), g.playlist = {id:c, name:DB.playlist[c].name}, b.push(g)
        }
    ...
    {
        var c = a.playlist.name.toLowerCase(), d = b.playlist.name.toLowerCase();
        return c > d ? 1 : c < d ? -1 : 0
    }}), WaitListJoinService = Service.extend({init:function ()
    {
        this._super()
    }, execute:function ()
    {
    ...
    {
        0 == a.data ? (UserListOverlay.waitListServiceComplete(!0),
            Room.updateUserPlaying()) : (1 == a.data ? Dialog.alert(Lang.alerts.maxDJs) : 2 == a.data ? Dialog.alert(Lang.alerts.lockedBooth) : 3 == a.data && Dialog.alert(Lang.alerts.cannotPlay), this.errorCallback && this.errorCallback(), Room.updateUserPlaying(), UserListOverlay.waitListServiceComplete(!1))
    }}), WaitListLeaveService = Service.extend({init:function ()
    {
        this._super()
    }, execute:function ()
    {
    ...
    SoundCloudSearchService = Class.extend({init:function ()
    {
        this.callback = null
    }, load:function (a, b, c, d)
    {
        ...
        $.jsonp({url:a, success:$.proxy(this.onComplete, this), error:$.proxy(this.onError, this)})
    }, onComplete:function (a)
    {
        for (var b = a.length, c = [], d = 0; d < b; ++d)
        {
            var e = a[d], f = {};
            if (e.streamable)f.cid = e.stream_url.split("/")[4]; else if (e.downloadable)f.cid = e.download_url.split("/")[4];
            else continue;
    ...
    }, onError:function ()
    {
        ...
        this.callback([])
    }}), SoundCloudFavoritesService = Class.extend({init:function ()
    {
        this.callback = null;
        this.hasConnected = !1
    }, load:function (a, b, c)
    {
        this.callback =
            c;
        var d = this;
        this.hasConnected ? SC.get("/me/favorites?offset=" + a * b + "&limit=" + b, {client_id:Playback.SCClientID, redirect_uri:Playback.SCRedirect}, function (a, b)
        {
            if (b)d.hasConnected = !1, Dialog.alert(b.message + "<br/>" + Lang.alerts.pleaseTryAgain, null, Lang.alerts.soundcloudError); else d.onComplete(a)
        }) : this.auth(a, b)
    }, auth:function (a, b)
    {
        var c = this;
        SC.connect(function ()
        {
            SC.get("/me/favorites?offset=" + a * b + "&limit=" + b, {client_id:Playback.SCClientID, redirect_uri:Playback.SCRedirect}, function (a, b)
            {
                b ? (c.hasConnected =
                    !0, Dialog.alert(b.message + "<br/>" + Lang.alerts.pleaseTryAgain, null, Lang.alerts.soundcloudError)) : (c.hasConnected = !0, c.onComplete(a))
            })
        })
    }, onComplete:function (a)
    {
        ...
        {
            var f = a[e], g = {};
            if (f.streamable)g.cid = f.stream_url.split("/")[4]; else if (f.downloadable)g.cid = f.download_url.split("/")[4]; else continue;
            var h = LS.media.lookup(g.cid);
            h ? c.push(h) : (g.image = f.artwork_url || f.avatar_url || "http://plug.dj/_/static/images/soundcloud_thumbnail.da976c8b.png", g.duration = f.duration /
                1E3, f.user ? g.author = f.user.username : (f = Utils.authorTitle(f.title), g.author = f.author || g.author), g.title = f.title, g.format = 2, g.id = 0, d.push(g))
        }
    ...
    }, onError:function ()
    {
        ...
        this.callback([])
    }}), SoundCloudTracksService = Class.extend({init:function ()
    {
        this.callback = null;
        this.hasConnected = !1
    }, load:function (a, b, c)
    {
        this.callback = c;
        var d = this;
        this.hasConnected ? SC.get("/me/tracks?offset=" + a * b + "&limit=" + b, {client_id:Playback.SCClientID, redirect_uri:Playback.SCRedirect},
            function (a, b)
            {
                if (b)d.hasConnected = !1, Dialog.alert(b.message + "<br/>" + Lang.alerts.pleaseTryAgain, null, Lang.alerts.soundcloudError); else d.onComplete(a)
            }) : this.auth(a, b)
    }, auth:function (a, b)
    {
        ...
        SC.connect(function ()
        {
            SC.get("/me/tracks?offset=" + a * b + "&limit=" + b, {client_id:Playback.SCClientID, redirect_uri:Playback.SCRedirect}, function (a, b)
            {
    ...
    }, onComplete:function (a)
    {
        ...
        onError:function ()
        {
            ...
            this.callback([])
        }}), SoundCloudSetsService = Class.extend({init:function ()
    {
        this.callback = null;
        this.hasConnected = !1
    }, load:function (a, b, c)
    {
        this.callback = c;
        var d = this;
        this.hasConnected ? SC.get("/me/playlists?offset=" + a * b + "&limit=" + b, {client_id:Playback.SCClientID, redirect_uri:Playback.SCRedirect}, function (a, b)
        {
            if (b)d.hasConnected = !1, Dialog.alert(b.message + "<br/>" + Lang.alerts.pleaseTryAgain, null, Lang.alerts.soundcloudError); else d.onComplete(a)
        }) :
            this.auth(a, b)
    }, auth:function (a, b)
    {
        ...
        SC.connect(function ()
        {
            SC.get("/me/playlists?offset=" + a * b + "&limit=" + b, {client_id:Playback.SCClientID, redirect_uri:Playback.SCRedirect}, function (a, b)
            {
                b ? (c.hasConnected = !0, Dialog.alert(b.message + "<br/>" + Lang.alerts.pleaseTryAgain, null, Lang.alerts.soundcloudError)) : (c.hasConnected = !0, c.onComplete(a))
            })
        })
    }, onComplete:function (a)
    {
        for (var b = a.length, c = [], d = 0; d < b; ++d)
        {
            ...
            for (var f = a[d].tracks.length, g = [], h = [], j = 0; j < f; ++j)
            {
                var k = a[d].tracks[j],
                    l = {};
                if (k.streamable)l.cid = k.stream_url.split("/")[4]; else if (k.downloadable)l.cid = k.download_url.split("/")[4]; else continue;
                var m = LS.media.lookup(l.cid);
                m ? g.push(m) : (l.image = k.artwork_url || k.avatar_url || "http://plug.dj/_/static/images/soundcloud_thumbnail.da976c8b.png", l.duration = k.duration / 1E3, k.user ? l.author = k.user.username : (k = Utils.authorTitle(k.title), l.author = k.author || l.author), l.title = k.title, l.format = 2, l.id = 0, h.push(l))
            }
            e.tracks = g.concat(h);
        ...
        onError:function ()
        {
            ...
            this.callback([])
        }}), PlaylistServiceFacade = Class.extend({init:function ()
    {
    ...
    {
        if (this.synced)null != this.syncCompleteCallback && this.syncCompleteCallback(); else
        {
            var a = new PlaylistSelectService(this.store.modified() || "0");
            a.successCallback = $.proxy(this.onSelectResponse,
                this);
    ...
    }, create:function (a, b, c, d)
    {
        ...
        !b || 0 == b.length ? (a = new PlaylistInsertService(a), a.successCallback = $.proxy(this.onItemResponse, this), a.errorCallback = $.proxy(this.onServiceError, this), this.curateAfter = c) : (a = new PlaylistMediaInsertService(0, a, b, 0), a.successCallback = $.proxy(this.onCreateInsertResponse, this), a.errorCallback = $.proxy(this.onServiceError, this), this.importCallback = d)
    }, remove:function (a)
    {
    ...
    }, onCreateInsertResponse:function (a)
    {
        ...
        }, onServiceError:function ()
        {
            MediaOverlay.hidePlaylistCreateProgress();
            MediaOverlay.hidePlaylistProgress()
        }}), MediaServiceFacade = Class.extend({init:function ()
    {
    ...
    {
        LS.media.write(a);
        0 < a.length ? this.sync() : this.syncCompleteCallback &&
            this.syncCompleteCallback()
    }, onServiceError:function ()
    {
    }}), PlaylistMediaServiceFacade = Class.extend({init:function (a)
    {
        this.id = a;
        this.confirmCount = this.updateCallback = null
    }, mediaInsert:function (a, b, c)
    {
        0 < a.length && (MediaOverlay.showPlaylistProgress(this.id), this.confirmCount = c, 50 > a.length ? (c = new PlaylistMediaInsertService(this.id, null, a, b), c.successCallback = $.proxy(this.onInsertResponse, this), c.errorCallback = $.proxy(this.onServiceError, this)) : (c = a.slice(0, 50), c = new PlaylistMediaInsertService(this.id,
            null, c, b), c.successCallback = $.proxy(this.onInsertResponse, this), c.errorCallback = $.proxy(this.onServiceError, this), a.splice(0, 50), this.mediaInsert(a, b)))
    }, mediaDelete:function (a)
    {
        if (0 < a.length)
        {
            ...
            50 > a.length ? (b = new PlaylistMediaDeleteService(this.id, a), b.successCallback = $.proxy(this.onDeleteResponse, this), b.errorCallback = $.proxy(this.onServiceError, this)) : (b = a.slice(0, 50), b = new PlaylistMediaDeleteService(this.id, b), b.successCallback = $.proxy(this.onDeleteResponse,
                this), b.errorCallback = $.proxy(this.onServiceError, this), a.splice(0, 50), this.mediaDelete(a))
        }
    ...
    }, mediaMove:function (a, b)
    {
        ...
        c.errorCallback = $.proxy(this.onServiceError,
            this)
    ...
    }, onServiceError:function ()
    {
        ...
        MediaOverlay.hidePlaylistCreateProgress();
        MediaOverlay.hidePlaylistProgress()
    }}), DJJoinService = Service.extend({init:function ()
    {
        this._super()
    }, execute:function ()
    {
        socket.execute("booth.join",
            this)
    }, parse:function (a)
    {
        0 != a.status && (1 == a.status ? Dialog.alert(Lang.alerts.maxDJs) : 2 == a.status ? Dialog.alert(Lang.alerts.lockedBooth) : 3 == a.status && Dialog.alert(Lang.alerts.cannotPlay), this.errorCallback && this.errorCallback(), Room.updateUserPlaying())
    }}), DJLeaveService = Service.extend({init:function ()
    {
        this._super()
    }, execute:function ()
    {
    ...
    }}), DJSkipService = Service.extend({init:function ()
    {
        this._super()
    },
        execute:function ()
        {
            Room.setSkipping();
            socket.execute("booth.skip", this)
        }, parse:function (a)
        {
            if (0 != a.status)this.onStatusError(a.status)
        }}), DJCurateService = Service.extend({init:function (a)
    {
        this._super();
        this.playlistID = a
    }, execute:function ()
    ...
    }, parse:function (a)
    {
        ...
        } else if (5 == a.status)Dialog.alert(Lang.alerts.tooManyItems); else this.onStatusError(a.status)
    }}), DJReplaceRestrictedMediaService = Service.extend({init:function (a, b)
    {
        this._super();
        this.item = a;
        this.replacement = b
    ...
    {
        console.log("DJReplaceRestrictedMediaService",
            this.item, " :: ", this.replacement);
        this.onResult({status:0})
    }, parse:function (a)
    {
        if (0 != a.status)this.onStatusError(a.status)
    }}), SearchServiceFacade = Class.extend({init:function ()
    {
    ...
    }}), CIDSearchService = Class.extend({init:function ()
    {
        this.callback = null
    }, load:function (a, b)
    {
    ...
    {
        this.callback([])
    }}), RoomSelectService = Service.extend({init:function (a, b)
    {
        this._super();
        this.query = a || "";
        this.cursor = b
    ...
    {
        if (0 == a.status)this.successCallback(a.data); else this.onStatusError(a.status)
    }}), RoomCreateService = Service.extend({init:function (a, b)
    {
        this._super();
        this.name = a;
        this.isPublic = b
    ...
    {
        0 == a.status ? (window.history.pushState(null, this.name, "/" + a.data + "/"), new RoomJoinService(a.data)) :
            Dialog.alert(Lang.alerts.createRoom + a.status)
    }}), RoomUpdateService = Service.extend({init:function (a, b, c)
    {
        this._super();
        this.id = a;
        this.name = b;
    ...
    {
        rpcGW.execute("moderate.update", this, {name:this.name, description:this.description})
    }, parse:function (a)
    {
        if (0 != a.status)this.onStatusError(a.status)
    }}), RoomPropsService = Service.extend({init:function (a, b, c, d)
    {
        this._super();
        this.id = a;
        this.options = {boothLocked:b, waitListEnabled:c, maxPlays:d, maxDJs:5}
    }, execute:function ()
    {
    ...
    {
        if (0 != a.status)this.onStatusError(a.status)
    }}), RoomVoteService = Service.extend({init:function (a, b)
    {
        this._super();
        this.positive = a;
        this.bc = !1;
    ...
    {
        if (0 != a.status)this.onStatusError(a.status)
    }}), RoomJoinService = Service.extend({init:function (a)
    {
        this._super();
        this.slug = a
    }, execute:function ()
    {
        ...
        Room.onMediaUpdate({});
        Lobby.timestamp = 0;
        Models.room.joined && Models.chat.clear();
    ...
    }, parse:function (a)
    {
        if (0 == a.status)
        {
            ...
            DB.ready("02:" + a.data.user.profile.id);
            ...
            API.isReady = !0
        } else if (404 == a.status)alert("Room " + this.slug + " does not exist"); else if (2 == a.status || 3 == a.status)
        {
        ...
        } else if (4 == a.status)window.location.href = "/" + encodeURIComponent(this.slug); else this.onStatusError(a.status)
    }}), RoomStateService = Service.extend({init:function ()
    {
        this._super()
    }, execute:function ()
    {
    ...
    }, parse:function (a)
    {
        if (0 == a.status)Models.room.setState(a.data),
            API.isReady = !0; else if (1 == a.status)new RoomJoinService(Models.room.data.id); else if (4 == a.status)window.location.href = "/" + encodeURIComponent(Models.room.data.id); else this.onStatusError(a.status)
    }}), YouTubeSearchService = Class.extend({init:function ()
    {
        this.callback = null;
    ...
    }, load:function (a, b, c, d)
    {
        ...
        $.jsonp({url:a + ("&start-index=" + (b * c + 1)) + ("&max-results=" + c) + "&format=5&paid-content=false&callback=?",
            success:$.proxy(this.onComplete, this), error:$.proxy(this.onError, this)})
    }, onComplete:function (a)
    {
        var b = a.feed.entry, c = [];
        if (b)try
        {
    ...
    }, deserialize:function (a, b)
    {
        ...
        a.media$group.media$credit &&
            a.media$group.media$credit.length && a.media$group.media$credit[0].$t ? c.author = a.media$group.media$credit[0].$t : a.author && a.author.length && a.author[0].name && (c.author = a.author[0].name.$t);
    ...
    }, onError:function (a, b)
    {
        ...
        !this.related && !this.importing && Dialog.alert(Lang.alerts.pleaseTryAgain, null, Lang.alerts.youtubeError);
        this.callback([])
    }}), YouTubeImportService = Class.extend({init:function ()
    {
        this.timeoutID = 0
    }, load:function (a, b, c)
    {
    ...
    }, onError:function (a, b)
    {
        ...
        this.callback([])
    }}), YouTubePlaylistService = Class.extend({init:function ()
    {
    }, load:function (a, b)
    {
        this.username = a;
    ...
    }, next:function ()
    {
        this.timeoutID && clearTimeout(this.timeoutID);
        this.resultCount = 0;
        $.jsonp({url:"http://gdata.youtube.com/feeds/api/users/" + this.username + "/playlists?v=2&alt=json&start-index=" + this.start + "&max-results=50&callback=?", success:$.proxy(this.onComplete, this), error:$.proxy(this.onError, this)})
    }, onComplete:function (a)
    {
        var b = !0;
        try
        ...
        b ? 0 < this.resultCount ? (this.start += 50, this.timeoutID = setTimeout($.proxy(this.next, this), 20)) : (this.data.sort(this.sortByName), this.data.unshift({name:"Favorites", username:this.username}), this.callback(this.data)) :
            (Dialog.alert(Lang.alerts.pleaseTryAgain, null, Lang.alerts.youtubeError), this.callback([]))
    ...
    }, deserialize:function (a)
    {
        ...
        {
            this.resultCount = a.length;
            for (var b = 0; b < this.resultCount; ++b)this.data.push({playlistID:a[b].yt$playlistId.$t, name:a[b].title.$t, count:a[b].yt$countHint.$t})
    ...
    YouTubeSuggestService = Class.extend({init:function ()
    {
        this.callback = null
    }, load:function (a)
    {
    ...
    }, onError:function (a, b)
    {
        ...
        console.log(a,
            b);
        this.callback([])
    }}), MediaSelectService = Service.extend({init:function (a)
    {
        this._super();
    ...
    {
        this.after = null;
        if (0 != a.status)this.onStatusError(a.status); else
        {
            for (var b = a.data.length; b--;)Utils.deserializeModified(a.data[b]);
            this.successCallback(a.data)
        }
    }}), MediaUpdateService = Service.extend({init:function (a)
    {
        this._super();
        this.item = {id:a.id, author:a.author, title:a.title}
    },
        execute:function ()
        {
        ...
        {
            if (0 == a.status)Utils.deserializeModified(a.data[0]), a.data[0].author = this.item.author, a.data[0].title = this.item.title, this.item = null, this.successCallback(a); else this.onStatusError(a.status)
        }}), PlaylistMediaInsertService = Service.extend({init:function (a, b, c, d)
    {
        this._super();
        this.id = a;
        this.name = b;
    ...
    }, parse:function (a)
    {
        ...
        } else if (2 == a.status)Dialog.alert(Lang.alerts.playlistLimit); else this.onStatusError(a.status);
        this.id = this.name = null
    }}), PlaylistMediaDeleteService = Service.extend({init:function (a, b)
    {
        this._super();
        this.id = a;
        ...
        }, parse:function (a)
        {
            ...
            if (0 == a.status)
            {
                ...
                this.successCallback(a);
                this.items = null
            } else if (2 == a.status)Dialog.alert(Lang.alerts.deleteLastItem); else this.onStatusError(a.status)
        }}), PlaylistMediaMoveService = Service.extend({init:function (a, b, c)
    {
        this._super();
        this.id = a;
        ...
        }, parse:function (a)
        {
            ...
            if (0 == a.status)Utils.deserializeModified(a.data), this.successCallback(a); else this.onStatusError(a.status)
        }}), SearchMediaCIDService = Service.extend({init:function (a)
    {
        this._super();
        this.cids = a
    }, execute:function ()
    ...
    }}), PlaylistSelectService = Service.extend({init:function (a)
    {
        this._super();
        this.after = Utils.convertNumberStringToUnixDateString(a)
    }, execute:function ()
    ...
    }}), PlaylistInsertService = Service.extend({init:function (a)
    {
        this._super();
        this.name = a
    }, execute:function ()
    ...
    {
        this.name = null;
        if (0 == a.status)Utils.deserializeModified(a.data), this.successCallback(a.data); else this.onStatusError(a.status)
    }}), PlaylistDeleteService = Service.extend({init:function (a)
    {
        this._super();
        this.id = a
    }, execute:function ()
    ...
    }, parse:function (a)
    {
        if (0 == a.status)
        {
            ...
            a.status)Dialog.alert(Lang.alerts.deleteActive); else this.onStatusError(a.status)
    }}), PlaylistRenameService = Service.extend({init:function (a, b)
    {
        this._super();
        this.id = a;
        this.name = b
    ...
    }, parse:function (a)
    {
        this.name = null;
        if (0 == a.status)Utils.deserializeModified(a.data), this.successCallback(a.data); else this.onStatusError(a.status)
    }}), PlaylistActivateService = Service.extend({init:function (a)
    {
        this._super();
        this.id = a
    }, execute:function ()
    ...
    }, parse:function (a)
    {
        if (0 == a.status)
        {
            for (var b = a.data.length; b--;)Utils.deserializeModified(a.data[b]);
            this.successCallback(a.data)
        } else if (2 == a.status)Dialog.alert(Lang.alerts.emptyPlaylist); else this.onStatusError(a.status)
    }}), ModerationForceSkipService = Service.extend({init:function (a)
    {
        this._super();
        this.historyID = a
    }, execute:function ()
    ...
    {
        if (0 != a.status)this.onStatusError(a.status)
    }}), ModerationKickUserService = Service.extend({init:function (a, b, c)
    {
        this._super();
        this.userID = a;
        this.reason = b;
    ...
    {
        if (0 != a.status)this.onStatusError(a.status)
    }}), ModerationAddDJService = Service.extend({init:function (a)
    {
        this._super();
        this.userID = a
    }, execute:function ()
    ...
    {
        if (0 == a.status)3 == a.data && Dialog.alert(Lang.alerts.modAddDJ); else this.onStatusError(a.status)
    }}), ModerationRemoveDJService = Service.extend({init:function (a)
    {
        this._super();
        this.userID = a
    }, execute:function ()
    ...
    {
        if (0 != a.status)this.onStatusError(a.status)
    }}), ModerationWaitListRemoveService = Service.extend({init:function (a)
    {
        this._super();
        this.userID = a
    }, execute:function ()
    {
        rpcGW.execute("moderate.remove_dj",
            this, this.userID)
    }, parse:function (a)
    {
        if (0 != a.status)this.onStatusError(a.status)
    }}), ModerationPermissionsService = Service.extend({init:function (a, b)
    {
        this._super();
        this.userID = a;
        this.permission = b
    ...
    {
        if (0 != a.status)if (1 == a.status)4 == this.permission ? Dialog.alert(Lang.alerts.cohostLimitError) : 3 == this.permission ? Dialog.alert(Lang.alerts.managerLimitError) : 2 == this.permission ? Dialog.alert(Lang.alerts.bouncerLimitError) :
            1 == this.permission && Dialog.alert(Lang.alerts.featuredDJLimitError); else if (2 == a.status)Dialog.alert(Lang.alerts.permissionError); else this.onStatusError(a.status)
    }}), ModerationBoothCleanupService = Service.extend({init:function ()
    {
        this._super()
    }, execute:function ()
    {
    ...
    {
        if (0 != a.status)this.onStatusError(a.status)
    }}), ModerationChatDeleteService = Service.extend({init:function (a)
    {
        this._super();
        this.chatID = a
    }, execute:function ()
    {
        rpcGW.execute("moderate.chat_delete",
            this, this.chatID)
    }, parse:function (a)
    {
        if (0 != a.status)this.onStatusError(a.status)
    }}), UserChangeDisplayNameService = Service.extend({init:function (a)
    {
        this._super();
        this.displayName = a
    }, execute:function ()
    {
        2 < this.displayName.length && -1 == this.displayName.indexOf("http://") ? socket.execute("user.change_name", this, this.displayName) : this.parse({status:5})
    }, parse:function (a)
    {
        if (0 == a.status)null != this.successCallback && this.successCallback(this.displayName); else this.onStatusError(a.status)
    }}), UserChangeAvatarService =
        Service.extend({init:function (a)
        {
            this._super();
            this.avatarID = a
        }, execute:function ()
        ...
        {
            if (0 == a.status)null != this.successCallback && this.successCallback(this.avatarID); else if (2 == a.status)AvatarOverlay.setSelectedAvatar(Models.user.data.avatarID), Dialog.alert(Lang.alerts.notEnoughPoints); else this.onStatusError(a.status)
        }}), UserLanguageUpdateService = Service.extend({init:function (a)
    {
        this._super();
        this.language = a
    }, execute:function ()
    {
        rpcGW.execute("user.set_language",
            this, this.language)
    }, parse:function (a)
    {
    ...
    }}), UserFanService = Service.extend({init:function (a, b)
    {
        this._super();
        this.value = a;
        this.userID = b
    ...
    {
        0 == a.status ? this.value ? (Models.chat.receive({type:"update", message:Lang.messages.fanOf.split("%NAME%").join($("<span/>").text(a.data).html()), language:Models.user.data.language}), Models.user.addFollowing(this.userID)) : (Models.chat.receive({type:"update", message:Lang.messages.unFanOf.split("%NAME%").join($("<span/>").text(a.data).html()), language:Models.user.data.language}),
    ...
    }}), UserDisplayNameAvailableService = Service.extend({init:function (a)
    {
        this._super();
        this.displayName = a
    }, execute:function ()
    {
        1 < this.displayName.length ? rpcGW.execute("user.name_available", this, this.displayName) : this.parse({status:5})
    }, parse:function (a)
    {
        null != this.successCallback && this.successCallback(0 == a.status)
    }}), UserStatusUpdateService = Service.extend({init:function (a)
    {
        this._super();
        this.userStatus = a
    }, execute:function ()
    ...
    {
        if (0 == a.status)null != this.successCallback && this.successCallback(this.displayName); else this.onStatusError(a.status)
    }}), UserGetByIdsService = Service.extend({init:function (a, b)
    {
        this._super();
        this.ids = a;
        this.pos = b
    ...
    }, parse:function (a)
    {
        if (0 == a.status)
        {
            ...
            for (var b = a.data.length, c = 0; c < b; ++c)a.data[c].owner =
                a.data[c].id == Models.room.data.owner, a.data[c].permission = Models.room.data.staff[a.data[c].id] ? Models.room.data.staff[a.data[c].id] : 0, Models.room.admins[a.data[c].id] ? a.data[c].admin = !0 : Models.room.ambassadors[a.data[c].id] && (a.data[c].ambassador = !0);
            null != this.successCallback && (this.pos ? this.successCallback(a.data[0], this.pos) : this.successCallback(a.data))
        } else this.onStatusError(a.status)
    }}), ChatModel = EventDispatcher.extend({init:function ()
    {
        this._super();
        this.initChatLanguages();
        this.lastChat = this.rateLimit =
            0
    }, ready:function ()
    {
    ...
    {
        var a, b;
        "off" != DB.settings.chatSound ? (a = "url(http://plug.dj/_/static/images/ButtonSoundOn.25ddac79.png)", b = "all" == DB.settings.chatSound ? Lang.chat.all : Lang.chat.mentions) : (a = "url(http://plug.dj/_/static/images/ButtonSoundOff.c40cf284.png)",
            b = Lang.chat.off);
        this.dispatchEvent("chatSoundUpdate", {image:a, text:b})
    }, sendChat:function (a)
    {
        if (!this.chatCommand(a))
        {
            var b = Date.now(), c = {}, d = !0;
            b - this.lastChat > 1E3 * this.rateLimit ? (this.lastChat = b, c.from = Models.user.data.username, c.fromID = Models.user.data.id, 0 == a.indexOf("/em") || 0 == a.indexOf("/me") ? (c.type = "emote", c.message = Utils.cleanTypedString(a.substr(3))) : (c.type = "message", c.message = Utils.cleanTypedString(a)), (-1 == c.message.indexOf("http://") && -1 == c.message.indexOf("https://") || Models.user.hasPermission(Models.user.FEATUREDDJ) ||
                6E5 < Date.now() - Models.room.joinTime) && socket.chat(a)) : (a = this.rateLimit - ~~((b - this.lastChat) / 1E3), 0 == a && (a = 1), c.message = 1 == a ? Lang.chat.limit1 : Lang.chat.limit.split("%SECS%").join("" + a), c.type = "update", d = !1);
    ...
    {
        var b = -1 < $("<span/>").html(a.message).text().indexOf("@" +
            Models.user.data.username) + " ";
        b && "emote" != a.type && (a.type = "mention");
        "log" == a.type && (a.type = "moderation");
        var c = {type:a.type}, d = a.from && ("message" == a.type || "mention" == a.type) ? ": " : "";
        if (a.from && (c.from = {value:a.from, "class":"chat-from"}, a.fromID))
        {
            "emote" != a.type ? Models.room.admins[a.fromID] ? c.from.class = "chat-from-admin" : Models.room.ambassadors[a.fromID] ? c.from.class = "chat-from-ambassador" : Models.room.data.owner == a.fromID ? c.from.class = "chat-from-host" : Models.room.data.staff[a.fromID] && 5 > Models.room.data.staff[a.fromID] ?
                4 == Models.room.data.staff[a.fromID] ? c.from.class = "chat-from-cohost" : 3 == Models.room.data.staff[a.fromID] ? c.from.class = "chat-from-manager" : 2 == Models.room.data.staff[a.fromID] ? c.from.class = "chat-from-bouncer" : 1 == Models.room.data.staff[a.fromID] && (c.from.class = "chat-from-featureddj") : c.from.class = a.fromID == Models.user.data.id ? "chat-from-you" : "chat-from" : c.from.class = "chat-from";
            Models.room.admins[a.fromID] ? c.from.admin = !0 : Models.room.ambassadors[a.fromID] ? c.from.ambassador = !0 : Models.room.data.owner ==
                a.fromID ? c.from.owner = !0 : Models.room.data.staff[a.fromID] && 5 > Models.room.data.staff[a.fromID] && (c.from.staff = Models.room.data.staff[a.fromID]);
            c.from.id = a.fromID;
            if (a.fromID != Models.user.data.id && (c.from.clickable = !0, c.deletable = !1, !Models.room.admins[a.fromID] && !Models.room.ambassadors[a.fromID]))if (Models.room.admins[Models.user.data.id] || Models.room.ambassadors[Models.user.data.id])c.deletable = !0; else if (Models.user.hasPermission(Models.user.BOUNCER) && (!Models.room.data.staff[a.fromID] || Models.room.data.staff[Models.user.data.id] >
                Models.room.data.staff[a.fromID]))c.deletable = !0;
            c.chatID = a.chatID
        }
        c.timestamp = Utils.getChatTimestamp(24 == DB.settings.chatTS);
        !DB.settings.chatTranslation || !a.translated ? c.text = d + this.parse(a.message) : 1 != a.translated ? (c.text = d + this.parse(a.translated), c.lang = {html:"<br/><em>[" + this.languageMap[a.language] + "]</em>", title:$(this.parse(a.message, !0)).text()}) : (c.text = d + this.parse(a.message), c.lang = {html:"<br/><em>[" + this.languageMap[a.language] + "]</em>", title:Lang.tooltips.couldNotTranslate});
        c.showTimestamp =
            DB.settings.chatTS;
        a.fromID && "off" != DB.settings.chatSound && (b ? c.sound = "mention" : "mentions" != DB.settings.chatSound && (c.sound = "chat"));
        this.dispatchEvent("chatReceived", {message:c})
    }, parse:function (a, b)
    {
        if (-1 == a.indexOf("http"))return a;
        var c = a.indexOf("http"), d = a.indexOf(" ", c), e = a.indexOf("\n", c);
        -1 < d && -1 < e ? d = Math.min(d, e) : -1 == d && (d = a.length);
        var e = a.substr(0, c), f = a.substr(d);
        return!b ? (c = a.substring(c, d), d = -1 < c.indexOf("plug.dj/") && -1 == c.indexOf("/about") && -1 == c.indexOf("blog.") && -1 == c.indexOf("support.") ?
            "_self" : "_blank", e + '<a href="' + c + '" target="' + d + '">' + c + "</a>" + this.parse(f)) : e + "[link]" + this.parse(f, b)
    ...
    }, chatCommand:function (a)
    {
        ...
        if ("/hd off" == a)return Playback.setHD(!1),
            !0;
        ...
        if (0 == a.indexOf("/ts "))return b = a.split(" ").pop(), DB.settings.chatTS = "12" == b ? 12 : "24" == b ? 24 : !1, this.dispatchEvent("timestampUpdate", {value:DB.settings.chatTS}), DB.saveSettings(), !0;
        if (0 == a.indexOf("/cap "))
        {
            if (a = parseInt(a.split(" ").pop()), 0 < a && 201 > a)return RoomUser.audience.gridData.avatarCap = a, RoomUser.redraw(),
                DB.settings.avatarcap = a, DB.saveSettings(), log(Lang.messages.cap.split("%COUNT%").join("" + a)), !0
        } else
        {
            ...
            if ("/stream on" == a)DB.settings.streamDisabled = !1, DB.saveSettings(), Playback.media && Playback.play(Playback.media, Playback.mediaStartTime), b = "Video/audio streaming enabled."; else if ("/stream off" == a)DB.settings.streamDisabled = !0, DB.saveSettings(), Playback.stop(), b = "<strong>Video/audio streaming has been stopped.</strong> Type <em>/stream on</em> to enable again.";
            else
            {
                if ("/clear" == a)return this.dispatchEvent("chatClear"), _gaq.push(["_trackEvent", "Chat", "Clear", Models.room.data.id]), !0;
                Models.room.ambassadors[Models.user.data.id] ? "/fixbooth" == a && (new ModerationBoothCleanupService, b = "Fixing Booth") : Models.room.admins[Models.user.data.id] && ("/fixbooth" == a ? (new ModerationBoothCleanupService, b = "Fixing Booth") : 0 == a.indexOf("/audience ") ? (a = parseInt(a.split(" ").pop()), 0 < a ? (RoomUser.testAddAvatar(a), b = "Adding " + a + " fake avatars to audience") : (RoomUser.clear(), RoomUser.setAudience(Models.room.getAudience()),
                    RoomUser.setDJs(Models.room.getDJs()), b = "Cleared fake avatars from audience")) : 0 == a.indexOf("/ping ") ? (DB.settings.showPings = "/ping on" == a ? !0 : !1, DB.saveSettings(), b = "Ping messages are " + (DB.settings.showPings ? "on" : "off")) : 0 == a.indexOf("/speed ") && (b = parseInt(a.split(" ").pop()), animSpeed = 0 < b ? b : 83, b = "Setting animation speed to " + animSpeed))
            }
        }
        return b ? (a = {type:"system"}, a.message = b, this.receive(a), !0) : !1
    }, initChatLanguages:function ()
    {
        this.uiLanguages = [];
        this.chatLanguages = [];
        this.uiLanguages.push({label:"Chinese (\u6f22\u8a9e)",
            value:"zh"});
        this.uiLanguages.push({label:"Deutsch", value:"de"});
        this.uiLanguages.push({label:"English", value:"en"});
        this.uiLanguages.push({label:"Espa\u00f1ol", value:"es"});
        this.uiLanguages.push({label:"Fran\u00e7ais", value:"fr"});
        this.uiLanguages.push({label:"Japanese (\u65e5\u672c\u8a9e)", value:"ja"});
        this.uiLanguages.push({label:"Korean (\ud55c\uad6d\uc778)", value:"ko"});
        this.uiLanguages.push({label:"Portugu\u00eas", value:"pt"});
        this.chatLanguages.push({label:"Afrikaans", value:"af"});
        this.chatLanguages.push({label:"Shqiptar",
            value:"sq"});
        this.chatLanguages.push({label:"\u0627\u0644\u0639\u0631\u0628\u064a\u0629", value:"ar"});
        this.chatLanguages.push({label:"\u0431\u0435\u043b\u0430\u0440\u0443\u0441\u043a\u0430\u0439", value:"be"});
        this.chatLanguages.push({label:"\u0431\u044a\u043b\u0433\u0430\u0440\u0441\u043a\u0438", value:"bg"});
        this.chatLanguages.push({label:"Catal\u00e0", value:"ca"});
        this.chatLanguages.push({label:"Hrvatski", value:"hr"});
        this.chatLanguages.push({label:"\u010cesk\u00fd", value:"cs"});
        this.chatLanguages.push({label:"Dansk",
            value:"da"});
        this.chatLanguages.push({label:"Nederlands", value:"nl"});
        this.chatLanguages.push({label:"Eesti", value:"et"});
        this.chatLanguages.push({label:"Tagalog", value:"tl"});
        this.chatLanguages.push({label:"Suomi", value:"fi"});
        this.chatLanguages.push({label:"Galacian", value:"gl"});
        this.chatLanguages.push({label:"\u03b5\u03bb\u03bb\u03b7\u03bd\u03b9\u03ba\u03ac", value:"el"});
        this.chatLanguages.push({label:"\u05e2\u05d1\u05e8\u05d9\u05ea", value:"iw"});
        this.chatLanguages.push({label:"\u0939\u093f\u0902\u0926\u0940",
            value:"hi"});
        this.chatLanguages.push({label:"Magyar", value:"hu"});
        this.chatLanguages.push({label:"\u00cdslenska", value:"is"});
        this.chatLanguages.push({label:"Indonesian", value:"id"});
        this.chatLanguages.push({label:"Gaeilge", value:"ga"});
        this.chatLanguages.push({label:"Italiano", value:"it"});
        this.chatLanguages.push({label:"Latvijas", value:"lv"});
        this.chatLanguages.push({label:"Lietuvos", value:"lt"});
        this.chatLanguages.push({label:"\u043c\u0430\u043a\u0435\u0434\u043e\u043d\u0441\u043a\u0438", value:"mk"});
        this.chatLanguages.push({label:"Melayu", value:"ms"});
        this.chatLanguages.push({label:"Maltija", value:"mt"});
        this.chatLanguages.push({label:"Norweigian", value:"no"});
        this.chatLanguages.push({label:"\u0641\u0627\u0631\u0633\u06cc", value:"fa"});
        this.chatLanguages.push({label:"Polski", value:"pl"});
        this.chatLanguages.push({label:"Rom\u00e2n", value:"ro"});
        this.chatLanguages.push({label:"\u0420\u0443\u0441\u0441\u043a\u0438\u0439", value:"ru"});
        this.chatLanguages.push({label:"\u0421\u0440\u043f\u0441\u043a\u0438",
            value:"sr"});
        this.chatLanguages.push({label:"Slovensk\u00fdch", value:"sk"});
        this.chatLanguages.push({label:"Slovenski", value:"sl"});
        this.chatLanguages.push({label:"Kiswahili", value:"sw"});
        this.chatLanguages.push({label:"Svenskt", value:"sv"});
        this.chatLanguages.push({label:"Thai (\u0e20\u0e32\u0e29\u0e32\u0e44\u0e17\u0e22)", value:"th"});
        this.chatLanguages.push({label:"T\u00fcrk", value:"tr"});
        this.chatLanguages.push({label:"\u0423\u043a\u0440\u0430\u0457\u043d\u0441\u044c\u043a\u0430", value:"uk"});
        this.chatLanguages.push({label:"Vi\u1ec7t", value:"vi"});
        this.chatLanguages.push({label:"Cymraeg", value:"cy"});
        this.chatLanguages.push({label:"\u05d9\u05d9\u05b4\u05d3\u05d9\u05e9", value:"yi"});
        this.languageMap = {};
        this.uiLanguageMap = {};
    ...
    }}), PlaylistModel = Class.extend({init:function ()
    {
        this.data = [];
    ...
    }, read:function ()
    {
        ...
        }}), SoundCloudPlaylistModel = Class.extend({init:function ()
    {
        this.updateCallback = null;
        this.data = [];
    ...
    }, "import":function ()
    {
        ...
        createPlaylist:function (a, b, c)
        {
            if (0 < b.length)
            {
                var d = a;
                1 < c && (d += " " + c);
                if (200 >= b.length)Models.playlist.facade.create(d, b); else
                {
                    var e = b.slice(0, 200);
        ...
        {
            this.facade.mediaUpdate({id:a, author:b, title:c})
        },
            mediaMove:function (a, b)
            {
            ...
            {
                this.storeRead && (this.currentFilter = this.currentLookup = this.facade = this.data = null, this.storeRead.readCallback = null, this.lookupCallback =
                    this.updateCallback = this.storeRead = this.storeRead.lookupCallback = null)
            }}), YouTubePlaylistModel = Class.extend({init:function ()
    {
        this.updateCallback = null;
        this.data = []
    }, load:function (a)
    ...
    {
        0 < this.data.length && (this.importMediaModel || (this.importMediaModel = new YouTubePlaylistMediaModel, this.importMediaModel.updateCallback = $.proxy(this.onImportMediaComplete,
            this)), this.importIndex = 0, this.importNextPlaylist(), setTimeout($.proxy(this.importAlert, this), 100))
    ...
    }, onImportMediaComplete:function (a)
    {
        ...
        ++this.importIndex < this.data.length && (this.timeoutID = setTimeout($.proxy(this.importNextPlaylist, this), 10))
    }, createPlaylist:function (a, b)
    {
        if (0 < a.length)
        {
            var c = this.data[this.importIndex].username ? "YouTube Favorites" :
                this.data[this.importIndex].name;
            1 < b && (c += " " + b);
            if (200 >= a.length)Models.playlist.facade.create(c, a); else
            {
                var d = a.slice(0, 200);
    ...
    }}), YouTubePlaylistMediaModel = Class.extend({init:function ()
    {
        this.updateCallback = null;
        this.data = []
    }, load:function (a)
    {
        this.data = [];
        this.ytp = a;
        a.playlistID ? Models.search.facade.ytPlaylistVideos(a.playlistID,
            $.proxy(this.onComplete, this)) : Models.search.facade.ytFavoriteVideos(a.username, $.proxy(this.onComplete, this))
    }, onComplete:function (a)
    {
        this.ytResults = a;
        if (0 < a.length)
    ...
    {
        for (var a = this.ytResults.length, b = 0; b < a; ++b)this.hash[this.ytResults[b].cid] || this.data.push(this.ytResults[b]);
        ...
        }, onCIDResult:function (a)
        {
            ...
            this.appendUnknown()
        }}), ModelRef = Class.extend({init:function ()
    {
        this.media =
            new MediaServiceFacade;
    ...
    }, playlistMedia:function (a)
    {
        ...
        return this._pm[a]
    }}), HistoryModel = Class.extend({init:function ()
    {
        this.data = [];
        this.hasLoaded = !1;
    ...
    {
        this.timeoutID && clearTimeout(this.timeoutID);
        if (this.hasLoaded)this.updateCallback(this.data); else
        {
            this.currentFilter = null;
            Search.clearInputs();
            var a = new HistorySelectService;
            a.successCallback = $.proxy(this.onSuccess, this);
            a.errorCallback = $.proxy(this.onError,
                this)
        }
    }, filter:function (a)
    {
    ...
    }, djAdvance:function (a)
    {
        this.currentID = a;
        this.hasLoaded = !1;
        MediaOverlay.selection == MediaOverlay.historyListView && $("#media-overlay").is(":visible") &&
        (this.timeoutID && clearTimeout(this.timeoutID), this.timeoutID = setTimeout($.proxy(this.load, this), 2E3))
    ...
    }, onError:function ()
    {
        ...
        this.hasLoaded = !1;
        this.updateCallback(this.data)
    }}), SearchModel = Class.extend({init:function ()
    {
        ...
        this.lastFormats = this.lastQuery = null;
    ...
    }, initCounts:function (a)
    {
        ...
        var a = 0, c = this.pageCounts.length;
    ...
    }, setPagination:function (a)
    {
        ...
        0 == this.page ? ($("#media-panel-pagination-left-button").css("opacity", 0.2), $("#media-panel-pagination-left-button").css("cursor", "default")) : ($("#media-panel-pagination-left-button").css("opacity",
            1), $("#media-panel-pagination-left-button").css("cursor", "pointer"));
    ...
    {
        !this.relatedSearch && "" != this.end && (!this.scFavoritesLookup && !this.scTracksLookup ? this.load(this.lastQuery, this.lastFormats,
            this.page + 1) : this.scFavoritesLookup ? this.loadSCFavorites(this.page + 1) : this.scTracksLookup && this.loadSCTracks(this.page + 1))
    }, onPrevPage:function ()
    {
    ...
    {
        this.relatedSearch && 0 == a.length ? this.load(this.relatedSearch.author + " " + this.relatedSearch.title,
            0) : (this.ytData = a, this.ytLoaded = !0, this.eliminateDuplicates())
    ...
    {
        if (a)for (var b =
            a.length, c = 0; c < b; ++c)if (!this.hash[a[c].cid])
        {
            var d = a[c].cid, e = LS.media.lookup(d);
    ...
    }}), RestrictedSearchModel = SearchModel.extend({initCounts:function ()
    {
    }, updateCounts:function ()
    {
    }, setPagination:function ()
    {
        ...
        }}), iTunesModel = Class.extend({load:function (a)
    {
        if (window.File && window.FileReader && window.FileList && window.Blob)
        {
            if (a = a.target.files, 0 < a.length)
            {
                var b = new FileReader, c = $.proxy(this.parse, this);
    ...
    }, parse:function (a)
    {
        ...
        parsePlaylists:function (a)
        {
            ...
            {
                for (var f = a[e].childNodes.length, g = null, h = null, j = 0; j < f; ++j)if ("key" === a[e].childNodes[j].nodeName)if ("Name" === a[e].childNodes[j].firstChild.nodeValue)g = a[e].childNodes[j].nextSibling.firstChild.nodeValue; else
                {
                    if ("Master" === a[e].childNodes[j].firstChild.nodeValue || "Distinguished Kind" ===
                        a[e].childNodes[j].firstChild.nodeValue)
                    {
                        h = g = null;
                        break
                    }
        ...
        }, parseLibrary:function (a)
        {
            ...
            e = c.length;
            for (b = 0; b < e; ++b)d[c[b].childNodes[2].firstChild.nodeValue] = {artist:c[b].childNodes[8].firstChild.nodeValue,
                name:c[b].childNodes[5].firstChild.nodeValue};
            return d
        }}), UserModel = Class.extend({init:function ()
    {
        this.data = {};
        this.followers = {};
        this.following = {};
    ...
    }, setData:function (a)
    {
        ...
        this.statuses = [
            {label:Lang.userStatus.available, value:0},
            {label:Lang.userStatus.away,
                value:1},
            {label:Lang.userStatus.working, value:2},
            {label:Lang.userStatus.sleeping, value:3}
        ];
    ...
    {
        var a = 0;
        if (Models.room.admins[this.data.id])a = 10; else if (Models.room.ambassadors[this.data.id])a = 9; else for (var b in Models.room.data.staff)if (this.data.id ==
            b)
        {
    ...
    }, onChangeDisplayNameSuccess:function (a)
    {
        ...
        Room.setUserMeta(this.data.username, this.data.djPoints + this.data.listenerPoints + this.data.curatorPoints, this.data.fans);
        Room.setUserName(this.data.username);
        if (Models.room.data && Models.room.userIsPlaying == this.data.id && Models.room.data.media)Room.onMediaUpdate({djName:this.data.username,
    ...
    }}), RoomModel = EventDispatcher.extend({init:function ()
    {
        this._super();
        this.clear()
    }, clear:function ()
    {
        ...
        this.admins = {};
        this.ambassadors = {};
        this.joined = this.userIsPlaying = this.userInBooth =
            !1;
        this.joinTime = null;
        this.roomScore = {positive:0, negative:0, curates:0, score:0.5};
        this.usernameLookup = {}
    }, setData:function (a)
    {
        this.clear();
        ...
        this.buildDJHash();
        for (a = this.data.users.length; a--;)this.data.users[a].owner = this.data.users[a].id == this.data.owner, this.data.users[a].permission = this.data.staff[this.data.users[a].id] ? this.data.staff[this.data.users[a].id] :
            0, this.userHash[this.data.users[a].id] = this.data.users[a], this.addUsernameLookup(this.data.users[a]);
        this.updateProps();
        this.updateChatRateLimit();
        RoomUser.setAudience(this.getAudience());
        RoomUser.setDJs(this.getDJs());
        this.dispatchEvent("voteUpdate", {noRefresh:!0, boothRefresh:!0});
        this.playMedia()
    }, updateChatRateLimit:function ()
    {
        var a = this.data.users.length;
        Models.chat.rateLimit = Models.user.hasPermission(Models.user.FEATUREDDJ) ? 0 : 199 < a ? 30 : 149 < a ? 20 : 99 < a ? 10 : 0
    }, setState:function (a)
    {
        var b = !0;
        if (this.data.media &&
            !a.media)b = !1; else if (!this.data.media && a.media)b = !1; else if (this.data.media && a.media && (this.data.media.cid != a.media.cid || this.data.historyID != a.historyID))b = !1;
        this.updateState(a);
        b || this.playMedia()
    ...
    }, updateState:function (a)
    {
        ...
        for (c in this.userHash)if (d[c])for (var e in d[c])this.userHash[c][e] = d[c][e]; else this.userLeave(c);
        this.data.staff = a.staff;
        this.djUpdate(a.djs);
        this.updateProps();
        this.dispatchEvent("voteUpdate", {noRefresh:!0, boothRefresh:!1});
    ...
    }, setBoothState:function (a)
    {
        ...
        try
        {
            for (b = this.data.users.length; b--;)this.data.users[b] && (this.data.users[b].vote = 0, this.data.users[b].curated = !1, this.userHash[this.data.users[b].id] &&
                (this.userHash[this.data.users[b].id].vote = 0))
        } catch (d)
        {
            console.error("ClearVotes", d)
        }
        b = !0;
        if (this.data.media && !a.media)b = !1; else if (!this.data.media && a.media)b = !1; else if (this.data.media && a.media && (this.data.media.cid != a.media.cid || this.data.historyID != a.historyID))b = !1;
        for (c in a)"users" != c && "mediaStartTime" != c && "djs" != c && (this.data[c] = a[c]);
        if (this.data.votes)for (c in this.data.votes)this.userHash[c] && (this.userHash[c].vote = this.data.votes[c]);
        if (this.data.curates)for (c in this.data.curates)this.userHash[c] &&
        (this.userHash[c].curated = this.data.curates[c]);
        this.data.mediaStartTime = Utils.convertUnixDateStringToNumberString(a.mediaStartTime);
        this.djUpdate(a.djs);
        ...
        Room.updateDJButton();
        this.dispatchEvent("voteUpdate", {noRefresh:!1, boothRefresh:!1});
    ...
    }, updateRelationships:function ()
    {
        for (var a in this.userHash)
        {
            ...
            if (!this.djHash[a] && (3 == this.userHash[a].relationship && 3 != b || 0 == b && 0 < this.userHash[a].relationship || 0 < b && 0 == this.userHash[a].relationship))RoomUser.userLeave(a),
    ...
    }, editStaff:function (a, b)
    {
        ...
        for (var c = this.data.users.length; c--;)if (this.data.users[c].id ==
            a)
        {
    ...
    }, getAudience:function (a)
    {
        ...
        b = d.concat(b);
        c &&
            !this.djHash[c.id] && b.unshift(c);
        return b
    }, sortAudienceRelationship:function (a, b)
    {
        if (a.relationship > b.relationship)return 1;
        if (a.relationship < b.relationship)return-1;
        var c = a.listenerPoints + a.djPoints + a.curatorPoints, d = b.listenerPoints + b.djPoints + b.curatorPoints;
        return c > d ? 1 : c < d ? -1 : 0
    ...
    {
        return a.permission < b.permission ? 1 : a.permission >
            b.permission ? -1 : a.username.toLowerCase() > b.username.toLowerCase() ? 1 : a.username.toLowerCase() < b.username.toLowerCase() ? -1 : 0
    }, getDJs:function ()
    {
        var a = [];
        if (this.data.djs)for (var b = this.data.djs.length, c = 0; c < b; ++c)this.userHash[this.data.djs[c].id] ? a.push(this.userHash[this.data.djs[c].id]) : console.error("getDJs Error :: " + this.data.djs[c].id + " does not exist in room users");
        return a
    }, getUsers:function ()
    {
    ...
    {
        var a = [], b;
        for (b in this.data.staff)this.userHash[b] &&
    ...
    }, getUserByID:function (a)
    {
        ...
        {
            var a = [];
            if (this.data.waitList)for (var b = this.data.waitList.length, c = 0; c < b; ++c)this.userHash[this.data.waitList[c]] ? a.push(this.userHash[this.data.waitList[c]]) : console.error("getWaitList Error :: " + this.data.waitList[c] + " does not exist in room users");
            return a
        }, getWaitListPosition:function (a)
        {
            a || (a = Models.user.data.id);
            for (var b = this.data.waitList.length; b--;)if (this.data.waitList[b] == a)return b + 1;
            return null
        }, getDJPlays:function (a)
        {
            return!this.data.boothLocked && 0 < this.data.maxPlays &&
                0 < this.data.waitList.length && this.djHash[a] ? Math.max(1, this.data.maxPlays - this.djHash[a].plays) : null
        }, buildDJHash:function ()
        {
            this.djHash = {};
            for (var a = this.data.djs.length; a--;)this.djHash[this.data.djs[a].id] = this.data.djs[a]
        ...
        {
            return this.userHash[a] ?
                this.userHash[a].username : null
        }, userJoin:function (a)
        {
            this.data.users && !this.userHash[a.id] && (a.owner = a.id == this.data.owner, a.permission = this.data.staff[a.id] ? this.data.staff[a.id] : 0, this.admins[a.id] ? a.admin = !0 : this.ambassadors[a.id] && (a.ambassador = !0), this.data.users.push(a), this.userHash[a.id] = a, this.updateRelationships(), RoomUser.userJoin(a), this.checkCurrentDJ(), this.updateRoomScore(), this.updateChatRateLimit(), UserListOverlay.updateUsers(), this.addUsernameLookup(a));
        ...
        }, userUpdate:function (a)
        {
            if (this.data.users)
            {
                ...
                if (b)
                {
                    ...
                    b.username != a.username && (this.removeUsernameLookup(b.username), this.addUsernameLookup(a), Models.chat.receive({type:"update", from:b.username, message:Lang.messages.changeName.split("%NAME%").join($("<span/>").text(a.username).html()), language:Models.user.data.language}));
                    ...
                    if (b.status != a.status && (b.id == Models.user.data.id || 1 < b.relationship))
                    {
                        ...
                        Models.chat.receive({type:"update", from:b.username, message:c + "&gt;", language:Models.user.data.language})
                    }
                    b.status = a.status;
                    this.checkCurrentDJ();
                    b.id == this.data.currentDJ && this.dispatchEvent("mediaUpdate",
                        {djName:b.username, media:this.data.media});
                    b.avatarID != a.avatarID && (b.avatarID = a.avatarID, RoomUser.userUpdate(b));
        ...
        {
            var a = this.userHash[Models.user.data.id], b = a.djPoints + a.listenerPoints + a.curatorPoints, b = a.id != this.data.currentDJ ? b + this.earnedPoints[a.id] : b + (this.earnedPoints[a.id].dj + this.earnedPoints[a.id].c);
            Room.setUserMeta(a.username, b, a.fans)
        }, voteUpdate:function (a)
        {
            this.userHash[a.id] ?
                (this.userHash[a.id].vote = a.vote, this.data.votes[a.id] = a.vote, this.earnedPoints[a.id] = 1, this.earnedPoints[this.data.currentDJ] && this.earnedPoints[this.data.currentDJ].v && (1 == a.vote && 1 != this.earnedPoints[this.data.currentDJ].v[a.id] ? (++this.earnedPoints[this.data.currentDJ].dj, this.earnedPoints[this.data.currentDJ].v[a.id] = 1) : -1 == a.vote && 1 == this.earnedPoints[this.data.currentDJ].v[a.id] && (--this.earnedPoints[this.data.currentDJ].dj, this.earnedPoints[this.data.currentDJ].v[a.id] = -1)), a.id != Models.user.data.id &&
                    this.dispatchEvent("voteUpdate", {noRefresh:!1, boothRefresh:this.djHash[a.id]}), this.updateRoomScore(), this.updateEarnedUserMeta()) : console.error("VoteUpdateError :: user " + a.id + " does not exist in room")
        }, curateUpdate:function (a)
        {
            this.userHash[a.id] ? (this.userHash[a.id].curated = a.curated, this.data.curates[a.id] = a.curated, this.earnedPoints[this.data.currentDJ] && ++this.earnedPoints[this.data.currentDJ].c, this.updateRoomScore(), this.updateEarnedUserMeta()) : console.error("CurateUpdateError :: user " +
                a.id + " does not exist in room")
        }, roomPropsUpdate:function (a)
        {
            this.data.boothLocked != a.boothLocked && (Models.chat.receive({type:"moderation", from:a.moderator, message:a.boothLocked ? Lang.messages.boothLocked : Lang.messages.boothUnlocked, language:Models.user.data.language}), this.data.boothLocked = a.boothLocked);
            this.data.maxPlays != a.maxPlays && (Models.chat.receive({type:"moderation", from:a.moderator, message:Lang.messages.maxPlays.split("%COUNT%").join("" + a.maxPlays), language:Models.user.data.language}), this.data.maxPlays =
                a.maxPlays);
            this.data.maxDJs != a.maxDJs && (Models.chat.receive({type:"moderation", from:a.moderator, message:Lang.messages.maxDJs.split("%COUNT%").join("" + a.maxDJs), language:Models.user.data.language}), this.data.maxDJs = a.maxDJs);
            this.data.waitListEnabled != a.waitListEnabled && (Models.chat.receive({type:"moderation", from:a.moderator, message:a.waitListEnabled ? Lang.messages.waitListEnabled : Lang.messages.waitListDisabled, language:Models.user.data.language}), this.data.waitListEnabled = a.waitListEnabled);
            this.data.waitListMode !=
                a.waitListMode && (this.data.waitListMode = a.waitListMode);
            Room.updateDJButton();
        ...
        {
            var b;
            this.data.media && (b = {dj:this.userHash[this.data.currentDJ], media:Utils.clone(this.data.media), score:Utils.clone(this.roomScore)}, this.applyEarnedPoints(a.earn));
            this.data.votes = {};
            this.data.curates = {};
            try
            {
            ...
            {
                console.error("History.djAdvance", f)
            }
            this.roomScore =
            {positive:0, negative:0, curates:0, score:0.5};
            try
            {
            ...
            try
            {
                this.data.media && this.data.currentDJ ? API.delayDispatch(API.DJ_ADVANCE,
                    {dj:this.userHash[this.data.currentDJ], media:this.data.media, lastPlay:b}) : API.delayDispatch(API.DJ_ADVANCE, {lastPlay:b})
            } catch (m)
            {
                console.error("API", m)
            }
        ...
        {
            for (var b = !1, c = {}, d = a.length; d--;)c[a[d].id] = !0;
            for (var d = this.data.djs.length, e = Array(d); d--;)!c[this.data.djs[d].id] && this.userHash[this.data.djs[d].id] && (b = !0, RoomUser.userJoin(this.userHash[this.data.djs[d].id])), e[d] = this.data.djs[d].id;
            this.data.djs = a;
            ...
            this.buildDJHash();
            b && (this.checkCurrentDJ(), RoomUser.setDJs(this.getDJs()), RoomUser.audience.refresh(), 5 > this.data.djs.length ? UserListOverlay.hide() : UserListOverlay.updateWaitList(), API.delayDispatch(API.DJ_UPDATE, this.getDJs()))
        }, updateRoomScore:function ()
        {
            if (this.data.media)
            {
                ...
                0 < a && (e = (b - c) / (2 *
                    a));
                this.roomScore = {positive:b, negative:c, curates:d, score:0.5 + e};
                this.dispatchEvent("scoreUpdate", this.roomScore);
                Models.history.roomScore(this.roomScore.score)
            } else this.roomScore = {positive:0, negative:0, curates:0, score:0.5}, this.dispatchEvent("scoreUpdate", this.roomScore);
        ...
        }, updateEarnedPoints:function ()
        {
            try
            {
                ...
                {
                    this.earnedPoints[this.data.currentDJ] = {dj:0, c:0, v:{}};
                    for (a in this.data.curated)this.earnedPoints[this.data.currentDJ].c +=
                        1;
        ...
        }, applyEarnedPoints:function (a)
        {
            try
            {
                ...
                {
                    if (this.userHash[b] &&
                        (b == this.data.currentDJ ? (this.userHash[this.data.currentDJ].djPoints += this.earnedPoints[this.data.currentDJ].dj || 0, this.userHash[this.data.currentDJ].curatorPoints += this.earnedPoints[this.data.currentDJ].c || 0) : 1 == this.earnedPoints[b] && ++this.userHash[b].listenerPoints, b == Models.user.data.id))Models.user.data.djPoints = this.userHash[b].djPoints, Models.user.data.listenerPoints = this.userHash[b].listenerPoints, Models.user.data.curatorPoints = this.userHash[b].curatorPoints
                } else if (this.userHash[this.data.currentDJ] &&
                    (this.userHash[this.data.currentDJ].curatorPoints += this.earnedPoints[this.data.currentDJ].c || 0), Models.user.data.id == this.data.currentDJ)Models.user.data.curatorPoints = this.userHash[Models.user.data.id].curatorPoints;
                Room.setUserMeta(Models.user.data.username, Models.user.data.djPoints + Models.user.data.listenerPoints + Models.user.data.curatorPoints, Models.user.data.fans)
            } catch (c)
            {
        ...
        {
            this.data.media && this.data.currentDJ ? (Playback.play(this.data.media, this.data.mediaStartTime), this.dispatchEvent("mediaUpdate", {djName:this.getUsername(this.data.currentDJ), media:this.data.media})) : (Playback.play(), this.dispatchEvent("mediaUpdate", {}))
        }, addUsernameLookup:function (a)
        {
            this.removeUsernameLookup(a.username);
            var b = a.username.charAt(0).toLowerCase();
            this.usernameLookup[b] ||
            (this.usernameLookup[b] = [], this.usernameLookup[b].longest = 0);
            this.usernameLookup[b].push({avatarID:a.avatarID, username:a.username});
        ...
        }, mentionLookup:function (a)
        {
            ...
            if (0 < c && 24 > c)
            {
                ...
                if (this.usernameLookup[d] && 0 < this.usernameLookup[d].length && c < this.usernameLookup[d].longest)for (c = this.usernameLookup[d].length; c--;)this.usernameLookup[d][c].username.toLowerCase() !=
                    a.toLowerCase() && 0 == this.usernameLookup[d][c].username.toLowerCase().indexOf(a.toLowerCase()) && b.push(this.usernameLookup[d][c])
            }
            b.length = Math.min(b.length, 10);
            b.sort(this.sortAudienceAlpha);
            return b
        }}), LobbyOverlayView = Class.extend({init:function ()
    {
    ...
    }, ready:function ()
    {
        ...
        {
            var a = "" + window.location, a = a.split("/");
            a.pop();
    ...
    }, show:function ()
    {
        ...
        (0 ==
            this.data.length || 15 < ~~(((new Date).getTime() - this.timestamp) / 1E3)) && this.load("", null)
    }, onKeyUp:function (a, b)
    {
        var c;
        if (window.event)c = window.event.keyCode; else if (b)c = b.which; else return!0;
        return 13 == c && ("" != this.searchInput.value || "" != this.lastQuery) ? (b.preventDefault(), this.onSubmitSearch(b), !1) : !0
    }, onSubmitSearch:function ()
    {
        if (("" != this.searchInput.value || "" != this.lastQuery) && this.searchInput.value != this.lastQuery)this.load(this.searchInput.value, null), _gaq.push(["_trackEvent", "Lobby Overlay",
    ...
    }, load:function (a, b)
    {
        if (!this.pendingLoad)
        {
            ...
            c.errorCallback = $.proxy(this.onRoomSelectError, this)
        }
    }, spin:function (a)
    {
        a ? this.lobbyPanelList.spin({lines:12, length:0, width:6,
            radius:20, color:"#000"}) : this.lobbyPanelList.spin(!1)
    }, onRoomSelectResponse:function (a)
    {
        ...
        if (0 == a.results.length)this.endOfList = !0; else if ((this.data = this.data.concat(a.results)) && 0 < this.data.length)
        {
            this.roomList || (this.roomList = $("<ul/>").addClass("room-list"));
            for (var a = this.data.length, c = b; c < a; ++c)
            {
                var d = this.data[c], e = $("<div/>").addClass("room-population").append($("<span/>").addClass("room-population-value").text(d.userCount)).append($("<span/>").addClass("room-population-label").html(Lang.lobby.population)),
                ...
                {
                    var h;
                    h = 4 == d.djCount ? Lang.lobby.slotAvailable : Lang.lobby.slotsAvailable.split("%COUNT%").join("" +
                        (5 - d.djCount));
    ...
    }, onRoomOut:function ()
    {
        ...
        }}), AvatarOverlayView = Class.extend({init:function ()
    {
        this.initAvatars();
    ...
    {
        var b = 0;
        "avatar-overlay-set-festival" == $(a.currentTarget).attr("id") &&
        (b = 1);
    ...
    }, draw:function ()
    {
        ...
        for (var a = this.avatarSets[this.selectedSet], b = a.length, c = 10, d = 0; d < b; ++d)
        {
            ...
            var e = a[d].avatars, f = e.length, g = Models.user.data.listenerPoints + Models.user.data.djPoints +
                Models.user.data.curatorPoints, h = Models.room.admins[Models.user.data.id] || Models.room.ambassadors[Models.user.data.id] ? !0 : !(a[d].required > g), j = $.browser.mozilla ? 155 : 157;
            5 < f && (j += 150);
    ...
    {
        $(a.currentTarget).data("available") &&
        (clearInterval(this.avatarAnimationID), this.overAvatar.css("left", 0), this.overAvatar = null, this.avatarFrame = 0)
    ...
    {
        Avatars =
        {audience:{w:150, h:150, sw:1650}, booth:{w:150, h:205, sw:1650}, dj:{w:170, h:220, sw:1870}};
        this.avatarSets = [];
        this.avatarSets.push(this.getOriginalSet());
        this.avatarSets.push(this.getFestivalSet())
    }, getOriginalSet:function ()
    {
        var a = {title:"0-99", required:0}, b = {title:"100-299", required:100}, c = {title:"300-699", required:300}, d = {title:"700-1499", required:700}, e = {title:"1500-2999", required:1500}, f = {title:"3000-6999", required:3E3}, g = {title:"7000-14999", required:7E3}, h = {title:"15000+", required:15E3};
        a.avatars =
            ["animal08", "animal05", "animal06", "animal02", "animal09"];
    ...
    {
        return[
            {title:"0-99", required:0, avatars:"bud02,bud08,bud01,bud09,bud03,bud10,bud06,bud11".split(",")},
            {title:"100+", required:100, avatars:["bud05", "bud07", "bud04"]}
        ]
    }}), AvatarRollover = Class.extend({init:function ()
    {
    ...
    }, showSimple:function (a, b, c)
    {
        ...
        if (!this.user || this.user.id != a.id)
        {
            ...
            this.notInRoom ? this.usertype.text(Lang.userStatus.notInRoom).css("color", "#cc0000") : this.user.admin ? this.usertype.text(Lang.rollover.admin + " " + d).css("color", "#42a5dc") : Models.room.ambassadors[a.id] ? this.usertype.text(Lang.rollover.ambassador + " " + d).css("color", "#9a50ff") : a.id == Models.room.data.owner ? this.usertype.text(Lang.rollover.host +
                " " + d).css("color", "#e90e82") : Models.room.data.staff[a.id] ? Models.room.data.staff[a.id] == Models.user.COHOST ? this.usertype.text(Lang.rollover.cohost + " " + d).css("color", "#e90e82") : Models.room.data.staff[a.id] == Models.user.MANAGER ? this.usertype.text(Lang.rollover.manager + " " + d).css("color", "#e90e82") : Models.room.data.staff[a.id] == Models.user.BOUNCER ? this.usertype.text(Lang.rollover.bouncer + " " + d).css("color", "#e90e82") : Models.room.data.staff[a.id] == Models.user.FEATUREDDJ && this.usertype.text(Lang.rollover.featuredDJ +
                " " + d).css("color", "#e90e82") : (this.usertype.text(d ? d.split("<").join("").split(">").join("") : Lang.userStatus.available), this.user.id == Models.user.data.id ? this.usertype.css("color", "#ffdd6f") : this.usertype.css("color", "#B0B0B0"));
            a = this.user.listenerPoints + this.user.djPoints + this.user.curatorPoints;
            this.user.id == Models.room.data.currentDJ ? isNaN(Models.room.earnedPoints[Models.room.data.currentDJ].dj + Models.room.earnedPoints[Models.room.data.currentDJ].c) || (a += Models.room.earnedPoints[Models.room.data.currentDJ].dj +
                Models.room.earnedPoints[Models.room.data.currentDJ].c) : isNaN(Models.room.earnedPoints[this.user.id]) || (a += Models.room.earnedPoints[this.user.id]);
            ...
            null != a ? (1 == a ? this.user.id == Models.room.data.currentDJ ? this.plays.text(Lang.rollover.lastPlay) : this.plays.text(Lang.rollover.playsRemaining1) : this.plays.text(Lang.rollover.playsRemaining.split("%COUNT%").join(a)), this.plays.show(),
                this.buttons.css("top", 90), this.requiredHeight = 90) : (this.plays.text(""), this.plays.hide(), this.buttons.css("top", 74), this.requiredHeight = 74);
            this.username.show();
            this.points.show();
            this.fans.show();
        ...
        this.div.width(a + 10);
        this.div.css("left", b - a / 2).css("top",
            c - this.requiredHeight - 5);
    ...
    }, showInfo:function (a)
    {
        ...
        if (this.notInRoom)
        {
            if (Models.room.admins[Models.user.data.id] || Models.room.ambassadors[Models.user.data.id] || !Models.room.data.staff[this.user.id] || Models.room.data.staff[this.user.id] <
                Models.room.data.staff[Models.user.data.id])b.push(this.getButton(Lang.rollover.modKick, "modkick")), Models.user.hasPermission(Models.user.MANAGER) && b.push(this.getButton(Lang.rollover.modBan, "modban"))
        } else
        {
            this.user.id != Models.user.data.id && (1 < this.user.relationship ? b.push(this.getButton(Lang.rollover.unfan, "unfan")) : b.push(this.getButton(Lang.rollover.becomeFan, "fan")), b.push(this.getButton(Lang.rollover.mention, "mention")));
            Models.room.djHash[this.user.id] && this.user.id == Models.user.data.id && Models.room.userIsPlaying &&
            b.push(this.getButton(Lang.rollover.skip, "skip"));
            if (this.user.id != Models.user.data.id && Models.user.hasPermission(Models.user.BOUNCER) && (Models.room.djHash[this.user.id] ? (Models.room.data.currentDJ == this.user.id && b.push(this.getButton(Lang.rollover.modSkip, "modskip")), b.push(this.getButton(Lang.rollover.modRemove, "modremovedj"))) : Models.room.data.djs.length < Models.room.data.maxDJs ? b.push(this.getButton(Lang.rollover.modAdd, "modadddj")) : Models.room.data.waitListEnabled && (null == Models.room.getWaitListPosition(this.user.id) ?
                b.push(this.getButton(Lang.rollover.modAddWait, "modadddjwait")) : b.push(this.getButton(Lang.rollover.modRemoveWait, "modremovedjwait"))), !this.user.owner && !this.user.admin && !this.user.ambassador))if ((Models.room.admins[Models.user.data.id] || Models.room.ambassadors[Models.user.data.id] || Models.user.hasPermission(Models.user.MANAGER) && (!Models.room.data.staff[this.user.id] || Models.room.data.staff[this.user.id] < Models.room.data.staff[Models.user.data.id])) && b.push(this.getButton(Lang.rollover.modPermissions,
                "modPerm")), Models.room.admins[Models.user.data.id] || Models.room.ambassadors[Models.user.data.id] || !Models.room.data.staff[this.user.id] || Models.room.data.staff[this.user.id] < Models.room.data.staff[Models.user.data.id])b.push(this.getButton(Lang.rollover.modKick, "modkick")), Models.user.hasPermission(Models.user.MANAGER) && b.push(this.getButton(Lang.rollover.modBan, "modban"));
            this.user.id == Models.user.data.id && (b.push(this.getButton(Lang.rollover.changeAvatar, "avatar")), b.push(this.getButton(Lang.dialog.userProfile,
                "userprofile")))
        }
        ...
        {
            "avatar" == a ? (AvatarOverlay.show(), _gaq.push(["_trackEvent", "User Rollover", "Change Avatar", Models.room.data.id])) : "userprofile" == a ? (Dialog.userProfile(), _gaq.push(["_trackEvent", "User Rollover",
                "User Profile", Models.room.data.id])) : "unfan" == a || "fan" == a ? (new UserFanService("fan" == a, this.user.id), _gaq.push(["_trackEvent", "User Rollover", "fan" == a ? "Fan" : "Unfan", Models.room.data.id])) : "mention" == a ? (Popout ? Popout.Chat.inputMention(this.user.username) : ((a = $(".chat-input input").attr("value")) || (a = ""), $(".chat-input input").attr("value", a + "@" + this.user.username + " ").focus()), _gaq.push(["_trackEvent", "User Rollover", "@ Mention", Models.room.data.id])) : "skip" == a ? (new DJSkipService, _gaq.push(["_trackEvent",
                "User Rollover", "Skip", Models.room.data.id, Playback.elapsed])) : "modskip" == a ? (new ModerationForceSkipService(Models.room.data.historyID), _gaq.push(["_trackEvent", "Moderation", "Force Skip", Models.user.data.id, Playback.elapsed])) : "modremovedj" == a ? (new ModerationRemoveDJService(this.user.id), _gaq.push(["_trackEvent", "Moderation", "Remove DJ", Models.user.data.id])) : "modadddj" == a ? (new ModerationAddDJService(this.user.id), _gaq.push(["_trackEvent", "Moderation", "Add DJ", Models.user.data.id])) : "modadddjwait" ==
                a ? (new ModerationAddDJService(this.user.id), _gaq.push(["_trackEvent", "Moderation", "Add Wait List", Models.user.data.id])) : "modremovedjwait" == a ? (new ModerationWaitListRemoveService(this.user.id), _gaq.push(["_trackEvent", "Moderation", "Remove Wait List", Models.user.data.id])) : "modkick" == a ? (Dialog.moderateUser(this.user, !1), _gaq.push(["_trackEvent", "Moderation", "Kick User", Models.user.data.id])) : "modban" == a ? (Dialog.moderateUser(this.user, !0), _gaq.push(["_trackEvent", "Moderation", "Ban User", Models.user.data.id])) :
                "modPerm" == a && (Dialog.userPermissions(this.user), _gaq.push(["_trackEvent", "Moderation", "User Permissions", Models.user.data.id]))
        }, hide:function (a)
        {
            if (this.isSimple || !0 == a)this.div.hide(), this.container.hide(), this.user = null, this.buttons.html(""), this.audience.css("cursor", "default"), this.hidden = !0, $("#avatar-info-modal-background").css("z-index", 64)
        }, onAvatarClick:function ()
        {
            this.user && this.showInfo()
        }}), UserListOverlayView = Class.extend({init:function ()
    {
        this.userList = new UserListView;
        $("#button-wait-list-join").click($.proxy(this.onWaitListJoinClick,
            this));
    ...
    }, show:function (a)
    {
        ...
        $("#overlay-container").show();
        var b;
        a ? (a = Models.room.getWaitList(), 1 != a.length && Lang.userList.waitTitle.split("%COUNT%").join("" + a.length), b = 1 != Models.room.data.waitListMode ? Models.room.getWaitListPosition() : null, b = null == b ? 1 != a.length ?
    ...
    }, updateUsers:function ()
    {
        ...
        {
            var a = Models.room.getRoomUsersSorted(), b;
            b = 1 != a.length ? Lang.userList.roomTitle.split("%COUNT%").join("" + a.length) :
    ...
    }, updateWaitList:function ()
    {
        if ($("#user-list-overlay").is(":visible") && this.userList.isWaitList)
        {
            var a = Models.room.getWaitList(), b = 1 != Models.room.data.waitListMode ? Models.room.getWaitListPosition() : null, b = null == b ? 1 != a.length ? Lang.userList.waitTitle.split("%COUNT%").join("" + a.length) : Lang.userList.waitTitle1 : Lang.userList.waitTitlePos.split("%POS%").join("" + b).split("%COUNT%").join("" +
                a.length);
    ...
    {
        null == Models.room.getWaitListPosition() ? ($("#button-wait-list-join").show(), $("#button-wait-list-leave").hide()) : ($("#button-wait-list-join").hide(), $("#button-wait-list-leave").show());
    ...
    }, waitListServiceComplete:function (a)
    {
        ...
        a ? ($("#button-wait-list-join").hide(),
            $("#button-wait-list-leave").show()) : ($("#button-wait-list-join").show(), $("#button-wait-list-leave").hide())
    ...
    {
        $("#user-list-overlay").is(":visible") && ($("#button-wait-list-join").hide(), $("#button-wait-list-leave").hide(), $("#wait-list-footer").spin({lines:12,
            length:0, width:5, radius:14}, "white"))
...
UserListView = Class.extend({init:function ()
{
...
var RoomView = Class.extend({init:function ()
{
...
{
    this.onMediaUpdate({djName:null, media:null});
...
{
    a ?
        (Models.room.userInBooth ? Models.room.userIsPlaying ? $("#your-next-play").text(Lang.room.yourNextTurn) : $("#your-next-play").text(Lang.room.onYourTurn) : $("#your-next-play").text(Lang.room.whenYouDJ), $("#up-next").text(a)) : ($("#your-next-play").text(Lang.room.nothingQueued), $("#up-next").text(Lang.room.nothingQueuedSuggestion))
...
{
    Playback.media && !Models.room.userIsPlaying && (0 < Models.playlist.data.length ? (0 < Models.playlist.data.length && PopMenu.show($("#button-add-this"),
        [Playback.media]), _gaq.push(["_trackEvent", "Actions", "Curate : Active : " + (Models.room.userInBooth ? "Booth" : "Audience"), Models.room.data.id])) : (Dialog.createPlaylist(null, !0), _gaq.push(["_trackEvent", "Actions", "Curate : Create : " + (Models.room.userInBooth ? "Booth" : "Audience"), Models.room.data.id])))
...
{
    !Models.room.data.boothLocked || Models.user.hasPermission(Models.user.FEATUREDDJ) ? 5 > Models.room.data.djs.length && 0 < Models.playlist.data.length &&
        0 < Models.playlist.getSelected().items.length ? ($("#button-dj-play").hide(), $("#button-dj-quit").hide(), $("#join-container").spin({lines:12, length:0, width:5, radius:14}, "white"), new DJJoinService, _gaq.push(["_trackEvent", "Actions", "DJ Join", Models.room.data.id])) : 5 == Models.room.data.djs.length ? Dialog.alert(Lang.alerts.maxDJs) : 0 == Models.playlist.data.length ? (Dialog.alert(Lang.alerts.createFirstPlaylist, $.proxy(MediaOverlay.showHelp, MediaOverlay)), _gaq.push(["_trackEvent", "Actions", "DJ Join - No Playlists",
...
}, onWaitListJoinClick:function (a)
{
    ...
    $("#join-container").spin({lines:12, length:0, width:5, radius:14}, "white");
...
}, onWaitListLeaveConfirm:function ()
{
    ...
    }, setUserName:function (a)
    {
        ...
        b.text(a);
        var a = 26, c = $("#user-name").width() - 6, d = 0, e = 0;
        do b.css("font-size", a), d = b.width(), e = b.height(), a -= 1; while (d > c && 12 < a);
    ...
    }, setDJName:function (a)
    {
        ...
        b.text(a);
        var a = 16, c = 0;
        do b.css("font-size", a), c = b.width(), b.height(), a -= 1; while (195 < c && 12 < a)
    ...
    {
        var a = Models.room.data.djs.length == Models.room.data.maxDJs, b = Models.room.data.waitListEnabled;
        Models.room.data.boothLocked ? Models.room.userInBooth ? this.showDJQuitButton(b) : Models.user.hasPermission(Models.user.FEATUREDDJ) ? a ? b && null != Models.room.getWaitListPosition() ? this.showWaitListLeaveButton() : b ? this.showWaitListJoinButton() : this.showLockButton() : this.showDJPlayButton(b) : b && null != Models.room.getWaitListPosition() ? this.showWaitListLeaveButton() : b ?
            this.showWaitListJoinButton() : this.showLockButton() : (b = b && a, Models.room.userInBooth ? this.showDJQuitButton(b) : b && null != Models.room.getWaitListPosition() ? this.showWaitListLeaveButton() : b ? this.showWaitListJoinButton() : this.showDJPlayButton(b));
        b ? this.showWaitListViewButton() : this.hideWaitListViewButton()
    }, showDJPlayButton:function (a)
    {
        this.hideDJButtons();
        a ? $("#button-dj-play").css("background-image", "url(" + Lang.ui.buttonDJPlayShort + ")").height(56).show() : $("#button-dj-play").css("background-image", "url(" +
            Lang.ui.buttonDJPlay + ")").height(81).show()
    ...
    {
        var a = "", a = 1 != Models.room.data.waitListMode ? Models.room.getWaitListPosition() : null, a = null == a ? 1 == Models.room.data.waitList.length ? Lang.userList.waitTitle1 : Lang.userList.waitTitle.split("%COUNT%").join("" + Models.room.data.waitList.length) : Lang.userList.waitTitlePos.split("%POS%").join("" +
            a).split("%COUNT%").join("" + Models.room.data.waitList.length);
    ...
    {
        var a = "http://plug.dj/" + Models.room.data.id, b;
        ...
        this.openPopup(1015, 495, "https://www.facebook.com/dialog/feed?app_id=216041638480603&redirect_uri=http%3A%2F%2Fplug.dj%2Ffbcallback&link=" +
            encodeURIComponent(a) + "&picture=" + encodeURIComponent("http://plug.dj/images/facebook01.png") + "&caption=" + encodeURIComponent(Lang.share.facebookCaption) + "&description=" + encodeURIComponent(c) + "&name=" + encodeURIComponent(b), "facebook");
    ...
    }}), RoomUserView = Class.extend({init:function ()
{
...
}, testAddAvatar:function (a)
{
    ...
    for (var c = "animal01,animal02,animal03,animal04,animal05,animal06,animal07,animal08,animal09,animal10,animal11,animal12,animal13,animal14,lucha01,lucha02,lucha03,lucha04,lucha05,lucha06,lucha07,lucha08,monster01,monster02,monster03,monster04,monster05,space01,space02,space03,space04,space05,halloween01,halloween02,halloween03,halloween04,halloween05,halloween06,halloween07,halloween08,halloween09,halloween10,halloween11".split(","),
             d = 0; d < a; ++d)
    {
        var e = this.random.integer(0, c.length - 1), e = {id:"$_" + ++this.testID, username:"" + this.testID + "_" + c[e], avatarID:c[e], vote:this.random.integer(0, 1), listenerPoints:this.random.integer(0, 1E3), djPoints:this.random.integer(0, 2500), curatorPoints:this.random.integer(0, 100)}, f = Math.random();
...
{
    0 < this.audienceArray.length ? this.audience.addUser(this.audienceArray.shift()) : (clearInterval(this.audienceIntervalID), Models.room.admins[Models.user.data.id] && console.log("Audience Render Complete :: " + this.audience.gridData.userCount + " avatars"))
}, setDJs:function (a)
{
    for (var b = a.length, c = 5; c--;)c < b && a[c] && this.audience.removeUser(a[c].id);
...
}, onVoteUpdate:function (a)
{
    ...
    {
        1 == Models.room.userHash[Models.user.data.id].vote ? (0 < Playback.duration ? $("#button-vote-negative").css("cursor", "pointer").css("background-image", "url(" + Lang.ui.buttonVoteNegative + ")").css("left", this.negativeVoteLeft) : $("#button-vote-negative").css("cursor", "default").css("background-image", "url(" + Lang.ui.buttonVoteNegativeDisabled + ")").css("left", this.negativeVoteLeft), $("#button-vote-positive").css("cursor", "default").css("background-image", "url(" + Lang.ui.buttonVotePositiveSelected +
            ")").css("left", 741)) : (-1 == Models.room.userHash[Models.user.data.id].vote ? $("#button-vote-negative").css("cursor", "default").css("background-image", "url(" + Lang.ui.buttonVoteNegativeSelected + ")").css("left", this.negativeVoteLeft) : 0 < Playback.duration ? $("#button-vote-negative").css("cursor", "pointer").css("background-image", "url(" + Lang.ui.buttonVoteNegative + ")").css("left", this.negativeVoteLeft) : $("#button-vote-negative").css("cursor", "default").css("background-image", "url(" + Lang.ui.buttonVoteNegativeDisabled +
            ")").css("left", this.negativeVoteLeft), $("#button-vote-positive").css("cursor", "pointer").css("background-image", "url(" + Lang.ui.buttonVotePositive + ")").css("left", 741));
        this.voteButtonsEnabled = !0
    }, updateVoteButtons:function ()
    {
        !Models.room.userIsPlaying && Models.room.data.media && (0 == Playback.duration || 5 < Playback.duration - Playback.elapsed ? this.enableVoteButtons() : this.voteButtonsEnabled && (-1 != Models.room.userHash[Models.user.data.id].vote && $("#button-vote-negative").css("cursor", "default").css("background-image",
            "url(" + Lang.ui.buttonVoteNegativeDisabled + ")").css("left", this.negativeVoteLeft), 1 != Models.room.userHash[Models.user.data.id].vote && $("#button-vote-positive").css("cursor", "default").css("background-image", "url(" + Lang.ui.buttonVotePositiveDisabled + ")").css("left", 741), this.voteButtonsEnabled = !1))
    }, onButtonVoteNegativeClick:function (a)
    {
        Playback.media && !Models.room.userIsPlaying && -1 != Models.room.userHash[Models.user.data.id].vote && 5 < Playback.duration - Playback.elapsed && (new RoomVoteService(!1, a), Models.room.userHash[Models.user.data.id].vote =
            Models.room.data.votes[Models.user.data.id] = -1, Models.room.updateRoomScore(), Models.room.userInBooth ? this.djBooth.refresh() : this.audience.refresh(), $("#button-vote-negative").css("cursor", "default").css("background-image", "url(" + Lang.ui.buttonVoteNegativeSelected + ")"), $("#button-vote-positive").css("cursor", "pointer").css("background-image", "url(" + Lang.ui.buttonVotePositive + ")"), _gaq.push(["_trackEvent", "Voting", "Negative : " + (Models.room.userInBooth ? "Booth" : "Audience"), Models.room.data.id]))
    }, onButtonVotePositiveClick:function (a)
    {
        if (Playback.media &&
            !Models.room.userIsPlaying && 1 != Models.room.userHash[Models.user.data.id].vote && (0 == Playback.duration || 5 < Playback.duration - Playback.elapsed))new RoomVoteService(!0, a), Models.room.userHash[Models.user.data.id].vote = Models.room.data.votes[Models.user.data.id] = 1, Models.room.updateRoomScore(), Models.room.userInBooth ? this.djBooth.refresh() : this.audience.refresh(), $("#button-vote-positive").css("cursor", "default").css("background-image", "url(" + Lang.ui.buttonVotePositiveSelected + ")"), $("#button-vote-negative").css("cursor",
            "pointer").css("background-image", "url(" + Lang.ui.buttonVoteNegative + ")"), _gaq.push(["_trackEvent", "Voting", "Positive : " + (Models.room.userInBooth ? "Booth" : "Audience"), Models.room.data.id])
    }}), reqAnimFrame = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || window.msRequestAnimationFrame, tickStart = Date.now(), animIntervalID, animSpeed = 83;
...
var MediaOverlayView = Class.extend({init:function ()
    {
        ...
        this.documentKeyDownProxy =
            $.proxy(this.onDocumentKeyDown, this);
        ...
        this.scImportPlaylistsView =
            new SoundCloudImportPlaylistsView;
    ...
    {
        this.display();
        this.firstTime ? (this.firstTime = !1, DB.settings.ph ? this.showPlaylist(Models.playlist.selectedPlaylistID) : (DB.settings.ph = !0, DB.saveSettings(), this.showHelp())) :
            (this.playlistMediaView.lockFirstItem = Models.room.userIsPlaying && this.selection == Models.room.data.playlistID, this.selection == Models.room.data.playlistID && (this.playlistMediaView.playlistID = this.selection, this.playlistMediaView.update()))
    }, showHelp:function ()
    {
        this.display();
        this.firstTime && (this.firstTime = !1);
        $("#media-panel .media-list").html("");
        this.helpView || (this.helpView = $("<div/>").addClass("playlist-overlay-help").append($("<span/>").append("<h1>" + Lang.help.header + "</h1><br/>").append("<h2>" +
            Lang.help.playing + "</h2>").append("<p>" + Lang.help.playing1 + "<br/>" + Lang.help.playing2 + '<br/><img src="http://plug.dj/_/static/images/ButtonActivatePlaylistSelected.4ecdc9fb.png"> ' + Lang.help.playing3 + '<br/><img src="http://plug.dj/_/static/images/ButtonActivatePlaylistReverse.5fb5278b.png"> ' + Lang.help.playing4 + "</p>").append("<h2>" + Lang.help.selecting + "</h2>").append("<p>" + Lang.help.selecting1 + "<br/>" + Lang.help.selecting2 + "<br/>" + Lang.help.selecting3 + "<br/>" + Lang.help.selecting4 + "<br/>" + Lang.help.selecting5 +
    ...
    }, posImportLabels:function ()
    {
        ...
        if (Main.isMac)$.browser.mozilla ? (a = -5, $("#media-import-youtube-button span").css("top", 8), $("#media-import-soundcloud-button span").css("top", 8), $("#tt-import-button span").css("top",
            8).css("left", 8), $("#itunes-import-button span").css("top", 8).css("left", 8)) : a = -2; else if (Main.isChrome)a = -2; else if ($.browser.msie || Main.isSafari)a = -7;
    ...
    {
        if (this.selection != this.historyListView || !this.historyListView.isRoom)this.historyListView.isRoom = !0, this.selection = this.historyListView, this.menuSelect(null), Models.history.hasLoaded ||
            this.showLoadProgress(), this.setPanelTitle(Lang.html.roomHistory, null), Models.history.load()
    ...
    {
        $("#media-overlay").is(":visible") && (this.currentListView == this.playlistMediaView ? this.playlistMediaView.updateTimeRemaining(a) :
            this.selection == this.historyListView && this.historyListView.refreshData())
    ...
    }, onPlaylistUpdate:function (a)
    {
        ...
        Main.isMac &&
            $.browser.mozilla && ($(".menu-playlist-item-name").css("top", 9), $(".menu-playlist-item-count").css("top", 24));
        this.updateQueueMeta()
    }, onPlaylistDelete:function (a)
    {
        if (a != Models.playlist.selectedPlaylistID)this.showPlaylist(Models.playlist.selectedPlaylistID); else
        {
    ...
    }, onPlaylistActivateClick:function (a)
    {
        ...
        if ((a = $(a.currentTarget).data("playlist")) && !a.selected)Models.room.userInBooth &&
            0 == Models.playlistMedia(a.id).data.length ? Dialog.alert(Lang.alerts.emptyPlaylist) : (Models.playlist.facade.activate(a.id), _gaq.push(["_trackEvent", "Actions", "Activate Playlist : Check"]))
    }, onPlaylistActionsActivateClick:function ()
    {
        ...
        {
            this.playlistMediaView.lockFirstItem =
                !1;
        ...
        }, menuSelect:function (a)
        {
            ...
            this.selection != this.searchView ? (Search.clearInputs(), $("#media-panel-pagination").hide(), this.currentListView != this.playlistMediaView &&
                this.currentListView != this.helpView ? $("#media-panel-count").show() : $("#media-panel-count").hide()) : ($("#media-panel-count").hide(), $("#media-panel-pagination").show());
            $("#media-panel .media-list").html("");
            $("#media-panel .media-list").scrollTop(0);
        ...
        }, onRelatedBackButtonClick:function ()
        {
            ...
            } else if (this.baseList == this.historyListView)this.onRoomHistoryClick(null), this.menuSelect($("#media-menu-room-history")); else if (this.baseList == this.ytImportPlaylistsView && this.selection == this.importMediaView)this.onYouTubeImport(this.ytImportPlaylistsView.username), $("#media-panel .media-list").scrollTop(0), this.ytImportPlaylistsView.setData(Models.youtubePlaylist.data); else if (this.baseList == this.ytImportPlaylistsView &&
                this.selection == this.searchView)this.onYouTubeImportMedia(this.importMediaView.ytp), $("#media-panel .media-list").scrollTop(0), this.importMediaView.setData(Models.youtubePlaylistMedia.data); else if (this.baseList == this.scImportPlaylistsView && this.selection == this.importMediaView)this.onSoundCloudSetsImport(), $("#media-panel .media-list").scrollTop(0), this.scImportPlaylistsView.setData(Models.soundcloudPlaylist.data); else if (this.baseList == this.scImportPlaylistsView && this.selection == this.searchView)$("#media-panel .media-list").scrollTop(0),
                this.onSoundCloudSetSelect(this.importMediaView.ytp); else
            {
        ...
        }, showPlaylist:function (a, b, c)
        {
            if (a)
            {
                ...
                this.pendingSelection = {id:b, jump:c};
        ...
        }, onSearch:function (a, b)
        {
            if (b)
            {
                if (this.currentListView == this.searchView)
                {
                    ...
                    this.history.push({title:$("#media-panel-title").text(), data:c})
                }
        ...
        {
            this.currentListView != this.searchPlaylistsView && (this.baseList = this.selection = this.currentListView = this.searchPlaylistsView, this.history = [], this.menuSelect(null), Search.searchInput.value = a, 0 == a.length ? $("#button-clear-search").hide() :
        ...
        }, setPanelTitle:function (a, b, c, d)
        {
            ...
            var f = 600;
            b ? d && null != this.baseList ? ($("#media-panel-title").css("left", 10 + e), $("#media-panel-icon").css("left", 5 + e), f -=
                55) : ($("#media-panel-icon").html('<div class="' + b + '"></div>'), $("#media-panel-icon").css("left", 5), $("#media-panel-title").css("left", 35), f -= 15) : $("#media-panel-title").css("left", 10 + e);
        ...
        }, updateTitle:function (a)
        {
            ...
            {
                b.text(a);
                var a = 19, c = $("#media-panel-title").width() -
                    3, d = $("#media-panel-title").height(), e = 0, f = 0;
                do b.css("font-size", a), e = b.width(), f = b.height(), a -= 1; while ((f > d || e > c) && 10 < a);
        ...
        {
            this.hideLoadProgress();
            this.loadProgress = (new Spinner({lines:12, length:0, width:6, radius:20, color:"#fff"})).spin();
        ...
        }, showPlaylistCreateProgress:function ()
        {
            ...
            this.createPlaylistProgress = (new Spinner({lines:8, length:0, width:3, radius:4, color:"#fff"})).spin();
        ...
        {
            this.createPlaylistProgress &&
            (this.createPlaylistProgress.stop(), this.createPlaylistProgress = null)
        }, showPlaylistProgress:function (a)
        {
            ...
            b && (this.hidePlaylistProgress(), this.playlistProgress = {target:b, spinner:(new Spinner({lines:8, length:0, width:3, radius:4, color:"#fff"})).spin()}, b.appendChild(this.playlistProgress.spinner.el), $(this.playlistProgress.spinner.el).css("left", 21).css("top", 20), $(b).find(".menu-playlist-item-activate-button").hide())
        },
        ...
        }, showRoomProgress:function ()
        {
            ...
            UserListOverlay.hide();
            $("#overlay-container").show();
            this.roomProgress = (new Spinner({lines:16, length:0, width:5, radius:40, color:"#fff"})).spin();
        ...
        {
            var a = Main.isMac && $.browser.mozilla ? 132 :
                130;
        ...
        {
            if (!Dialog.isOpen && "BODY" == $(document.activeElement)[0].nodeName)if (17 == a.keyCode || Main.isMac && (91 == a.keyCode || 93 == a.keyCode || 224 == a.keyCode))this.ctrlDown = !0; else
            {
                if (this.ctrlDown &&
                    65 == a.keyCode)return a.preventDefault(), this.currentListView.selectAll(), _gaq.push(["_trackEvent", "Actions", "Key", "Select All"]), !1;
        ...
        {
            if (17 == a.keyCode || Main.isMac && (91 == a.keyCode || 93 == a.keyCode || 224 == a.keyCode))this.ctrlDown = !1; else if (!this.ctrlDown && (8 == a.keyCode || 46 == a.keyCode))return a.preventDefault(), !1;
            return!0
        }}), PlaybackView = Class.extend({init:function ()
    {
        this.player = null;
        this.lastVolume = this.volume = 50;
        this.elapsed = this.duration = 0;
        this.buffering = this.isMuted = !1;
        this.fadeVolumeIntervalID = this.timeUpdateIntervalID = this.bufferPerc =
            0;
    ...
    }, ready:function ()
    {
        ...
        $(function ()
        {
            $("#slider").slider({orientation:"horizontal", value:Playback.volume, range:"min", animate:!1, start:function ()
            {
    ...
    }, initSC:function ()
    {
        ...
        if (SC)try
        {
            SC.initialize({client_id:this.SCClientID, redirect_uri:this.SCRedirect})
        } catch (a)
        {
        } else 10 > this.scTries++ && setTimeout($.proxy(Playback.initSC, this), 250)
    ...
    }, play:function (a, b, c)
    {
        ...
        if (this.media)
        {
            ...
            if (!DB.settings.streamDisabled)
            {
                ...
                var d = 4 > this.elapsed ? 0 : this.elapsed, a = {quality:"high", wmode:"opaque", allowScriptAccess:"always", allowFullScreen:"false", bgcolor:"#000000"};
                if (1 == this.media.format)this.buffering =
                    !1, this.yto = {id:this.media.cid, volume:this.volume, seek:d, quality:DB.settings.hdVideo ? "hd720" : ""}, a = isiOS ? "pdjytiOS2" : DB.settings.yt5 || isAndroid ? "pdjyt5" : "pdjytf", -1 < window.location.href.indexOf("pepper.plug.dj") ? a += "pepper" : -1 < window.location.href.indexOf("http://localhost") && (a += "local"), a = $('<iframe id="yt-frame" frameborder="0" src="http://pdj-youtube.appspot.com/' + a + '.html"></iframe>'), a.load($.proxy(this.onYTFrameLoaded, this)), $("#playback-container").append(a); else if (2 == this.media.format)if (this.durationChecked =
                    !0, SC)
                {
                    $("#playback-container").append($("<div/>").attr("id", "scplayer"));
                    swfobject.embedSWF("http://plug.dj/_/static/swf/line.f8a8c437.swf", "scplayer", "484", "271", "10", null, null, a, {id:"scplayer"});
                    var e = this;
                    SC.whenStreamingReady(function ()
                    {
                        if (e.media)
                        {
                            var a = SC.stream(e.media.cid, {autoPlay:true, volume:e.volume, position:d * 1E3, whileloading:$.proxy(e.onSCLoading, e), onfinish:$.proxy(e.playbackComplete, e), ontimeout:$.proxy(e.onSCTimeout, e)});
                            e.player = a
                        }
                    })
    ...
    {
        this.media &&
        (this.refreshButton.hide(), this.hdOffButton.hide(), this.hdOnButton.hide())
    ...
    }, setHD:function (a)
    {
        a ? (DB.settings.hdVideo = !0, this.media && "1" == this.media.format && WML.ytx("setPlaybackQuality=hd720"),
            DB.saveSettings(), Models.chat.receive({type:"update", message:Lang.messages.hdOn})) : (DB.settings.hdVideo = !1, this.media && "1" == this.media.format && WML.ytx("setPlaybackQuality=medium"), DB.saveSettings(), Models.chat.receive({type:"update", message:Lang.messages.hdOff}))
    ...
    {
        console.log("SCTimeout :: " +
            a.success + ", error type = " + a.error.type)
    }, playbackComplete:function ()
    {
        this.ignoreComplete || (this.stop(), $("#playback-container").append($("<div/>").attr("id", "syncing")), swfobject.embedSWF(this.syncSwf, "syncing", "484", "271", "10", null, null, {quality:"high", wmode:"opaque", allowScriptAccess:"always", allowFullScreen:"false", bgcolor:"#000000"}, {id:"syncing"}))
    ...
    }, updateTimeRemaining:function ()
    {
        ...
        var a = this.duration - this.elapsed, b;
    ...
    }, setVolume:function (a)
    {
        ...
        try
        {
            "2" == this.media.format && this.player ? this.player.setVolume(a) : WML.ytx("setVolume=" +
                a)
        } catch (b)
        {
        }
    ...
    }, previewMute:function (a)
    {
        ...
        a && !this.isMuted ?
            (this.lastVolume = this.volume, a = 1E3 * (0.15 / this.volume), this.fadeVolumeIntervalID = setInterval($.proxy(this.fadeToMute, this), a)) : !a && this.isMuted && (a = 1E3 * (0.15 / this.lastVolume), this.fadeVolumeIntervalID = setInterval($.proxy(this.fadeFromMute, this), a))
    }, fadeToMute:function ()
    {
    ...
    {
        this.volume == this.lastVolume ? clearInterval(this.fadeVolumeIntervalID) :
            this.setVolume(Math.min(this.lastVolume, this.volume + 1));
        $("#slider").slider("value", this.volume)
    ...
    }}), MediaListView = Class.extend({init:function ()
    {
        ...
        this.dragIcon =
            $("<div/>").addClass("drag-media-icon").append($("<span/>").addClass("drag-media-label"));
        this.dragIcon.hide();
        this.data = []
    }, setData:function (a)
    {
    ...
    {
        var a = this.getList(), b, c = this.data.length;
    ...
    }, onCellPress:function (a)
    {
        ...
        var b = !1, c = $(a.currentTarget), d = c.data("index"), e = this.data[d], f = c.data("selected");
        if (!this.lockFirstItem || 0 < d || this.data[0].cid !=
            Models.room.data.media.cid)
        {
            this.noModifier = !1;
            this.lastClickedIndex = d;
            if (!a.shiftKey && !a.ctrlKey)-1 == this.baseSelectedCellIndex || !f ? this.selectSingleCell(c) : this.noModifier = !0, (new Date).getTime() - this.doubleClickTime < this.doubleClickThreshold && e == this.doubleClickCellData && (b = !0, this.doubleClickCell(c)), this.doubleClickTime = (new Date).getTime(), this.doubleClickCellData = e; else if (a.ctrlKey && !a.shiftKey)this.baseSelectedCellIndex = d, this.toggleCell(c); else if (a.shiftKey && !a.ctrlKey)-1 == this.baseSelectedCellIndex ?
                (this.baseSelectedCellIndex = d, this.toggleCell(c)) : this.baseSelectedCellIndex > d ? this.selectMultipleCells(this.baseSelectedCellIndex, d) : this.baseSelectedCellIndex < d && this.selectMultipleCells(d, this.baseSelectedCellIndex); else if (a.ctrlKey && a.shiftKey)
            {
                var g;
                if (-1 == this.baseSelectedCellIndex)this.selectSingleCell(c); else if (d > this.baseSelectedCellIndex)if (this.selectedCells[this.data[this.baseSelectedCellIndex].cid])this.selectMultipleCells(d, this.baseSelectedCellIndex); else
                {
                    e = this.data.length;
                    for (c =
                             0; c < e; ++c)if (this.selectedCells[this.data[c].cid])
                    {
                        g = c;
                        break
                    }
                    this.selectMultipleCells(d, g)
                } else if (d < this.baseSelectedCellIndex)if (this.selectedCells[this.data[this.baseSelectedCellIndex].cid])this.selectMultipleCells(this.baseSelectedCellIndex, d); else
                {
    ...
    }, onCellRelease:function (a)
    {
        if (!this.isDragging)
        {
            ...
            {
                var a = $(a.currentTarget), b = a.data("index");
    ...
    }, selectSingleCell:function (a)
    {
        ...
        this.toggleCell(a);
        this.lastClickedCell = a;
        var b = a.data("selected"), a = a.data("index");
        ...
        {
            var b = a.data("selected"), c = a.data("index");
            b ? (a.data("selected", !1), a.removeClass("selected-cell"), 0 == c % 2 ? (a.addClass("alternate-cell"), a.removeClass("selected-alternate-cell")) : a.removeClass("alternate-cell"), delete this.selectedCells[this.data[c].cid]) :
                (a.data("selected", !0), 0 == c % 2 ? (a.addClass("selected-alternate-cell"), a.removeClass("alternate-cell")) : a.addClass("selected-cell"), this.selectedCells[this.data[c].cid] = this.data[c])
        ...
        }, onDragUpdate:function ()
        {
            ...
            {
                var a = $("#media-menu-playlist-container"), b = a.scrollTop(), c = a.offset().top - 48, d = a.offset().left, e = d + a.width(), f = a.height();
        ...
        {
            var b = a.offset().top,
                c = a.offset().left, d = c + a.width(), e = a.scrollTop(), f = a.height();
        ...
        }, onMoveToTopClick:function (a)
        {
            ...
            if (this.playlistID)
            {
                var a = this.data[$("body").find(a.target).data("index")], b = this.getSelectedCells();
                if (0 < b.length)
                {
        ...
        }, onDeleteClick:function (a)
        {
            a.stopImmediatePropagation();
            Search.hideSuggestions();
            var b = $("body").find(a.target).data("index"), a = this.data[b], c = this.getSelectedCells();
            if (Models.room.userIsPlaying &&
                Models.room.data.playlistID == this.playlistID && 0 == b && a.id == Models.room.data.media.id)Dialog.alert(Lang.alerts.deleteCurrentPlaylist); else if (Models.room.userInBooth && Models.playlist.selectedPlaylistID == this.playlistID && Models.playlist.getSelected().items.length == c.length)Dialog.alert(Lang.alerts.deleteLastItem); else if (0 < c.length)
            {
        ...
        }, onAddClick:function (a)
        {
            ...
            if (Models.playlist.selectedPlaylistID)
            {
                ...
                if (0 < c.length)
                {
                    for (var d = c.length, e = !1; d--;)if (c[d].id ==
                        b.id)
                    {
                        e = !0;
                        break
        ...
        }, onRelatedClick:function (a)
        {
            a.stopImmediatePropagation();
            Search.hideSuggestions();
            var a = this.data[$("body").find(a.target).data("index")], b = $("#media-panel .media-list");
            b.html("");
            b.scrollTop(0);
            !a.format || 1 == a.format ? (MediaOverlay.onSearch(a.author + " - " + a.title, !0), Models.search.loadYTRelated(a), _gaq.push(["_trackEvent", this.playlistID ? "Playlist" : this.isRoom ? "Room History" : "Search", "Related : YouTube"])) : 2 == a.format && (MediaOverlay.onSearch(a.author,
                !0), Models.search.load(a.author, "2", 0), _gaq.push(["_trackEvent", this.playlistID ? "Playlist" : this.isRoom ? "Room History" : "Search", "Related : SoundCloud"]));
            PopMenu.hide()
        }}), PlaylistMediaListView = MediaListView.extend({setData:function (a, b)
    {
    ...
    {
        this.nowPlayingCell &&
        this.nowPlayingCell.text(Utils.formatTime(a, !1))
    }, getList:function ()
    {
        return $("<ul/>").addClass("playlist-media")
    }, getRow:function (a, b)
    {
        var c = $("<div/>").addClass("thumbnail").append($("<img/>").attr("src", a.image)), d = $("<div/>").addClass("meta").append($("<span/>").addClass("author").text(a.author)).append($("<span/>").addClass("title").text(" - " + a.title)).append($("<span/>").addClass("duration").text(Utils.formatTime(a.duration, !1))), e = $("<div/>").addClass("actions").append($("<div/>").addClass("edit").attr("title",
    ...
    {
        this._super(a);
        var a = $("#media-panel .media-list"), b = a.offset().top, c = a.scrollTop();
        if (this.currentDragCell)
        {
            var d = 0, e = this.currentDragCell.data("index");
            !this.lockFirstItem || 0 < e ? (this.targetDropIndex = e, d = this.currentDragCell.offset().top, this.mouseY >= d + this.currentDragCell.height() / 2 ? (d = d - b + c + this.currentDragCell.height(), this.targetDropIndex = e == this.lastClickedIndex - 1 ? this.lastClickedIndex : e +
                1) : (d = d - b + c, this.targetDropIndex = e == this.lastClickedIndex + 1 ? this.lastClickedIndex : e), this.dragRowLine.css("top", d)) : 0 == e && (d = this.currentDragCell.offset().top - b + c + this.currentDragCell.height(), this.targetDropIndex = 1, this.dragRowLine.css("top", d))
        }
    ...
    }}), SearchMediaListView = MediaListView.extend({getList:function ()
    {
        return $("<ul/>").addClass("search")
    }, getRow:function (a, b)
    {
        var c = $("<div/>").addClass("thumbnail").append($("<img/>").attr("src", a.image)), d = $("<div/>").addClass("meta").append($("<span/>").addClass("author").text(a.author)).append($("<span/>").addClass("title").text(" - " + a.title)).append($("<span/>").addClass("duration").text(Utils.formatTime(a.duration,
            !1)));
    ...
    }}), SearchPlaylistsMediaListView = MediaListView.extend({getList:function ()
    {
        return $("<ul/>").addClass("search-playlists")
    }, getRow:function (a, b)
    {
        var c = $("<div/>").addClass("thumbnail").append($("<img/>").attr("src",
            a.image)), d = $("<div/>").addClass("meta").append($("<span/>").addClass("author").text(a.author)).append($("<span/>").addClass("title").text(" - " + a.title)).append($("<span/>").addClass("duration").text(Utils.formatTime(a.duration, !1))), e = "MediaOverlay.showPlaylist('" + a.playlist.id + "', '" + a.id + "', '1')";
    ...
    }, draw:function ()
    {
        this._super();
        $.browser.mozilla && (Main.isMac ? $(".media-list .playlists").css("background-position", "0 1px") : $(".media-list .playlists").css("background-position", "0 2px"))
    }}), RestrictedSearchListView = MediaListView.extend({setData:function (a)
    {
        this.data = a;
    ...
    }, getRow:function (a, b)
    {
        ...
        0 != a.id && (c = LS.media.exists(a.id) ? "(O) " :
            "(P) ");
        var d = $("<div/>").addClass("thumbnail").append($("<img/>").attr("src", a.image)), c = $("<div/>").addClass("meta").append($("<span/>").addClass("author").text(c + a.author)).append($("<span/>").addClass("title").text(" - " + a.title)).append($("<span/>").addClass("duration").text(Utils.formatTime(a.duration, !1))), d = $("<li/>").addClass("restricted-search-media-item").data("index", b).mouseenter($.proxy(this.onCellOver, this)).mouseleave($.proxy(this.onCellOut, this)).mousedown($.proxy(this.onCellPress,
    ...
    }, onCellOver:function (a)
    {
        a = $(a.currentTarget);
        0 == a.data("index") %
            2 ? (a.addClass("selected-alternate-cell"), a.removeClass("alternate-cell")) : (a.removeClass("cell"), a.addClass("selected-cell"))
    }, onCellOut:function (a)
    {
        var a = $(a.currentTarget), b = a.data("index");
    ...
    }}), HistoryListView = MediaListView.extend({setData:function (a)
    {
        this.data = a;
        for (var a = this.data.length, b = 0; b < a; ++b)this.data[b].idx = b, this.data[b].media.idx =
            b;
    ...
    {
        var c = $("<div/>").addClass("thumbnail").append($("<img/>").attr("src",
            a.media.image)), d = $("<div/>").addClass("meta").append($("<span/>").addClass("author").text(a.media.author)).append($("<span/>").addClass("title").text(" - " + a.media.title)).append($("<span/>").addClass("username").text(a.user.un)), e = $("<span/>").addClass("timestamp").text(Utils.convertTimestampToReadableDate(a.timestamp, !0)), f = $("<span/>").addClass("score").text(Lang.history.score.split("%VALUE%").join(~~(100 * a.room.score) + "%"));
        a.cid == Models.room.data.historyID && (this.timestampCell = e, this.scoreCell =
            f);
        d.append(f).append(e);
    ...
    }, draw:function ()
    {
        this._super();
        $("#playlist-filter").show()
    }}), YouTubeImportPlaylistsView = Class.extend({init:function ()
    {
        this.data = []
    }, setData:function (a)
    {
    ...
    SoundCloudImportPlaylistsView = YouTubeImportPlaylistsView.extend({onCellRelease:function (a)
    {
        a = $(a.currentTarget).data("index");
        Models.soundcloudPlaylist.selectedIndex = a;
        MediaOverlay.onSoundCloudSetSelect(this.data[a])
    }}), GridData = Class.extend({init:function ()
    {
        this.rows = 26;
        this.columns = 232;
        this.cellSize = 10;
    ...
    }, clear:function ()
    {
        ...
        this.addZone(3, {x:0, y:this.rows - 5, w:this.columns, h:5});
        this.addZone(4, {x:0.375 * this.columns, y:this.rows - 4, w:this.columns / 4, h:2});
        this.addZone(5, {x:110, y:this.rows - 1, w:12, h:1});
        for (a = 0; a < this.rows; ++a)
        {
            ...
            this.invalidateCellsInBounds({x:0, y:a, w:d + 2, h:1});
            this.invalidateCellsInBounds({x:this.columns - d - 2, y:a, w:d + 2, h:1})
        }
        this.invalidateRoomElements()
    }, invalidateRoomElements:function ()
    {
        this.invalidateCellsInBounds({x:27, y:4, w:16, h:1});
        this.invalidateCellsInBounds({x:29, y:5, w:17, h:1});
        this.invalidateCellsInBounds({x:31, y:6, w:17, h:1});
        this.invalidateCellsInBounds({x:33, y:7, w:17, h:1});
        this.invalidateCellsInBounds({x:35, y:8, w:17,
            h:1});
        this.invalidateCellsInBounds({x:36, y:9, w:18, h:1});
        this.invalidateCellsInBounds({x:39, y:10, w:18, h:1});
        this.invalidateCellsInBounds({x:41, y:11, w:18, h:1});
        this.invalidateCellsInBounds({x:41, y:12, w:19, h:1});
        this.invalidateCellsInBounds({x:0, y:13, w:63, h:2});
        this.invalidateCellsInBounds({x:3, y:15, w:50, h:1});
        this.invalidateCellsInBounds({x:198, y:0, w:1, h:1});
        this.invalidateCellsInBounds({x:194, y:1, w:2, h:1});
        this.invalidateCellsInBounds({x:192, y:2, w:4, h:1});
        this.invalidateCellsInBounds({x:186, y:3, w:9,
            h:1});
        this.invalidateCellsInBounds({x:180, y:4, w:14, h:1});
        this.invalidateCellsInBounds({x:177, y:5, w:18, h:1});
        this.invalidateCellsInBounds({x:174, y:6, w:20, h:1});
        this.invalidateCellsInBounds({x:172, y:7, w:21, h:1});
        this.invalidateCellsInBounds({x:169, y:8, w:23, h:1});
        this.invalidateCellsInBounds({x:167, y:9, w:30, h:1});
        this.invalidateCellsInBounds({x:165, y:10, w:50, h:1});
        this.invalidateCellsInBounds({x:163, y:11, w:50, h:1});
        this.invalidateCellsInBounds({x:160, y:12, w:50, h:1});
        this.invalidateCellsInBounds({x:158,
            y:13, w:50, h:1});
        this.invalidateCellsInBounds({x:155, y:14, w:60, h:1});
        this.invalidateCellsInBounds({x:150, y:15, w:60, h:1});
        this.invalidateCellsInBounds({x:147, y:16, w:60, h:1});
        this.invalidateCellsInBounds({x:165, y:17, w:100, h:1});
        this.invalidateCellsInBounds({x:165, y:18, w:5, h:1});
        this.invalidateCellsInBounds({x:164, y:19, w:5, h:1});
        this.invalidateCellsInBounds({x:163, y:20, w:5, h:1});
        this.invalidateCellsInBounds({x:158, y:21, w:9, h:1});
        this.invalidateCellsInBounds({x:156, y:22, w:10, h:2})
    }, addZone:function (a, b)
    {
        this.zones.push({id:a, bounds:b})
    }, addUser:function (a)
    {
        if (!this.userMap[a.id])
        {
            ...
            {
                var b = [], c = this.getZone(a);
                do b = this.getCellsInZone(c--); while (0 == b.length && 0 < c);
                if (0 == b.length)
                {
                    c = 4;
                    do b = this.getCellsInZone(c--); while (0 == b.length && 0 < c);
                ...
                for (a = 1; 4 >= a; ++a)this.decrementCellsInBounds({x:b.c - a, y:b.r - 2 * a, w:2 * a + 1, h:3 * a});
                return!0
            }
            return!1
        }
        ...
        }, removeUser:function (a)
        {
            if (this.userMap[a])
            {
                ...
                if (b)
                {
                    ...
                    var c;
                    for (c = 1; 4 >= c; ++c)this.incrementCellsInBounds({x:b.c - c, y:b.r - 2 * c, w:2 * c + 1, h:3 * c})
                }
        ...
        }, getZone:function (a)
        {
            ...
            var b =
                this.random.float(0, 1), a = a.djPoints + a.listenerPoints + a.curatorPoints;
        ...
        }, getCellsInZone:function (a, b)
        {
            for (var c = [], d = this.zones.length, e = 0; e < d; ++e)
            {
                ...
                if (f.id == a)
                {
                    for (d = f.bounds.y; d < f.bounds.y + f.bounds.h; ++d)for (e = f.bounds.x; e < f.bounds.x + f.bounds.w; ++e)(0 < this.getPriorityLevelForCellAt(d, e) || b) && c.push({c:e, r:d});
                    break
                }
            }
        ...
        {
            for (var b = a.y; b < a.y + a.h; ++b)if (!(0 >
                b || b >= this.rows))for (var c = a.x; c < a.x + a.w; ++c)0 > c || c >= this.columns || this.invalidateCellAt(b, c)
        ...
        }, decrementCellAt:function (a, b)
        {
            ...
            this.priorityGrid[a][b] -=
                this.getAdjustment(c[c.length - 1])
        ...
        }}), DialogView = Class.extend({init:function ()
    {
    ...
    {
        this.closeDialog();
        this.context = {items:a, playlistID:b};
        ...
        {
            this.closeDialog();
            this.context = {item:a, playlistID:b};
        ...
        {
            var a =
                $("#dialog-edit-media").find("input"), b = a[0].value;
            a[0].value = a[1].value;
            a[1].value = b;
        ...
        }, preview:function (a)
        {
            ...
            if (a.cid)
            {
                ...
                1 == a.format ? (this.previewFrame = $('<iframe id="ytprv" frameborder="0"/>').css("position", "absolute"), this.previewFrame.load($.proxy(this.onYTPreviewLoaded, this)), b.append($("<div/>").addClass("dialog-body").height(c + 38).append(this.previewFrame)), c = isiOS ? "pdjytpiOS" :
                    ...
                    a.cid + '&amp;show_comments=false&amp;auto_play=true&amp;color=55B9FF" type="application/x-shockwave-flash"></embed></object>', b.append($("<div/>").addClass("dialog-body").height(c).html(a)), this.showDialog(b))
            }
        }, onYTPreviewLoaded:function ()
        {
            this.context && document.getElementById("ytprv").contentWindow.postMessage(JSON.stringify({id:this.context.cid, volume:Playback.lastVolume}), "http://pdj-youtube.appspot.com")
        ...
        }, onRestrictedSearchUpdate:function (a)
        {
            ...
            var b = "", b = a && 0 < a.length ? Lang.dialog.chooseAlternative : Lang.dialog.noAlternative;
        ...
        {
            $(a.currentTarget).find("input").attr("checked") ||
            $(a.currentTarget).css("background-color", "#2A2A2A").css("border", "1px solid #888")
        ...
        }, submitCreateRoom:function ()
        {
            ...
            if (0 < a.length)
            {
                var b = null != $(".dialog-create-room-public").find("input").attr("checked"), b = !0;
        ...
        }, roomInfo:function ()
        {
            ...
            Models.user.hasPermission(Models.user.COHOST) || Models.room.admins[Models.user.data.id] ? (this.context = Models.room.data, this.ignoreEnter = !0, a.append($("<div/>").addClass("dialog-body").append($("<form/>").submit("return false").append(this.getInputField("name",
                Lang.dialog.roomName, Lang.dialog.roomName, Models.room.data.name, 32)).append(this.getCheckBox(Lang.dialog.boothLocked, "boothLocked", Models.room.data.boothLocked)).append(this.getCheckBox(Lang.dialog.waitListEnabled, "waitListEnabled", Models.room.data.waitListEnabled)).append(this.getMenu(Lang.dialog.maxPlays, "maxPlays", [0, 1, 2, 3, 4, 5], Models.room.data.maxPlays)).append(this.getTextArea("description", Lang.dialog.roomDescription, Models.room.data.description || "", 80, 4, 512)))).append(this.getCancelButton()).append(this.getSubmitButton(Lang.dialog.save))) :
                Models.user.hasPermission(Models.user.MANAGER) ? (this.context = Models.room.data, a.append($("<div/>").addClass("dialog-body").append($("<form/>").submit("return false").append(this.getInputField("name", Lang.dialog.roomName, Lang.dialog.roomName, Models.room.data.name, 32, !0)).append(this.getCheckBox(Lang.dialog.boothLocked, "boothLocked", Models.room.data.boothLocked)).append(this.getCheckBox(Lang.dialog.waitListEnabled, "waitListEnabled", Models.room.data.waitListEnabled)).append(this.getMenu(Lang.dialog.maxPlays,
                    ...
                    (this.context = null, a.append($("<div/>").addClass("dialog-body").append($("<div/>").append(this.getInputField("name", Lang.dialog.roomName, Lang.dialog.roomName, Models.room.data.name, 32, !0)).append($("<span/>").attr("id", "room-info-boothlocked-label").addClass("room-info-cbsub-label").text(Models.room.data.boothLocked ? Lang.dialog.boothIsLocked : Lang.dialog.boothIsUnlocked)).append($("<span/>").attr("id", "room-info-waitlist-label").addClass("room-info-cbsub-label").text(Models.room.data.waitListEnabled ?
                        Lang.dialog.waitListIsEnabled : Lang.dialog.waitListIsDisabled)).append($("<span/>").attr("id", "room-info-maxplays-label").addClass("room-info-cbsub-label").text(Lang.dialog.maxPlaysCount.split("%COUNT%").join(Models.room.data.maxPlays))).append($("<span/>").addClass("room-info-description-label").text(Lang.dialog.roomDescription)).append($("<div/>").addClass("room-info-description-container").append($("<div/>").addClass("frame-background")).append($("<div/>").addClass("room-info-description-scroll").append($("<span/>").addClass("room-info-description").html(Models.chat.parse(Models.room.data.description ||
        ...
        }, submitRoomInfo:function ()
        {
            if (this.context)
            {
                var a = $("#dialog-checkbox-boothLocked").is(":checked"), b = $("#dialog-checkbox-waitListEnabled").is(":checked"), c = parseInt($("#dialog-menu-maxPlays").val());
                if (Models.user.hasPermission(Models.user.COHOST))
                {
                    var d = $("#dialog-room-info").find("input").attr("value"), e = $("#dialog-room-info").find("textarea").val();
                    if (0 == d.length)return;
                    (d != this.context.name || e !=
                        this.context.description) && new RoomUpdateService(this.context.id, d, e)
                }
        ...
        }, userProfile:function ()
        {
            ...
            this.showDialog($("<div/>").attr("id", "dialog-user-profile").addClass("dialog").css("left", Main.LEFT +
        ...
        {
            var a = $("#dialog-menu-language").val(), b = $("#dialog-user-profile").find("input").attr("value"), c = $("#dialog-menu-userstatus").val(), d = $("#dialog-checkbox-html5player").is(":checked");
        ...
        }, alert:function (a, b, c, d)
        {
            ...
            this.showDialog($("<div/>").attr("id", "dialog-alert").addClass("dialog").css("left", Main.LEFT + (Main.WIDTH - 300) / 2).css("top", 267).append(this.getHeader(c ||
                Lang.dialog.alert)).append($("<div/>").addClass("dialog-body").append(this.getMessage(a))).append(this.getSubmitButton(Lang.dialog.ok)))
        ...
        {
            this.closeDialog();
            this.context = {user:a, ban:b};
            this.submitFunc = $.proxy(this.submitModerateUser, this);
            var c = b ? Lang.dialog.banUser : Lang.dialog.kickUser, d = b ? Lang.dialog.banUserPrompt.split("%NAME%").join(a.username) : Lang.dialog.kickUserPrompt.split("%NAME%").join(a.username);
        ...
        {
            this.closeDialog();
            this.context = {user:a, permission:a.permission};
            this.submitFunc = $.proxy(this.submitUserPermissions, this);
            var b = "", c = "#FFF";
            0 < a.permission && (c = "#e90e82", b = 4 == a.permission ? Lang.rollover.cohost : 3 == a.permission ? Lang.rollover.manager :
                2 == a.permission ? Lang.rollover.bouncer : 1 == a.permission ? Lang.rollover.featuredDJ : Lang.rollover.none);
        ...
        }, getPermissionMenuItem:function (a, b, c, d)
        {
            ...
            {
                $("#dialog-user-permissions-description").html("<span>" +
                    (4 == Dialog.context.permission ? Lang.dialog.cohostDescription : 3 == Dialog.context.permission ? Lang.dialog.managerDescription : 2 == Dialog.context.permission ? Lang.dialog.bouncerDescription : 1 == Dialog.context.permission ? Lang.dialog.featuredDJDescription : Lang.dialog.noneDescription) + "</span>")
            });
        ...
        }, submitUserPermissions:function ()
        {
            ...
            _gaq.push(["_trackEvent", "Dialog", "User Permissions", Models.user.data.id + " -> " +
                this.context.user.id, this.context.permission]);
            this.closeDialog()
        }, importTT:function (a)
        {
            try
            {
                var b = JSON.parse(a), a = [], c;
                for (c in b)"Unknown" != b[c].metadata.artist && "Untitled" != b[c].metadata.song && a.push(b[c].metadata);
                if (0 < a.length)this.context = a, this.submitFunc = $.proxy(this.submitImportTT, this), this.showDialog($("<div/>").attr("id", "dialog-import-tt").addClass("dialog").css("left", Main.LEFT + (Main.WIDTH - 300) / 2).css("top", 272).append(this.getHeader("Turntable Import")).append($("<div/>").addClass("dialog-body").append(this.getMessage("Import " +
                    a.length + " songs?"))).append(this.getCancelButton()).append(this.getSubmitButton("Import"))); else throw"No Items";
            } catch (d)
            {
        ...
        }, ttCreatePlaylist:function (a, b)
        {
                ...
                a.length)
            {
                var c = "TT Import";
                1 < b && (c += " " + b);
                if (200 >= a.length)Models.playlist.facade.create(c, a), this.closeDialog(); else
                {
                    var d = a.slice(0, 200);
                    Models.playlist.facade.create(c, d);
                    a.splice(0, 200);
        ...
        }, submitSoundCloudImport:function (a)
        {
            ...
            "dialog-soundcloud-favorites-button" ==
                a ? (MediaOverlay.onSoundCloudImport("favorites"), Models.search.loadSCFavorites(0)) : "dialog-soundcloud-tracks-button" == a ? (MediaOverlay.onSoundCloudImport("tracks"), Models.search.loadSCTracks(0)) : "dialog-soundcloud-sets-button" == a && (MediaOverlay.onSoundCloudSetsImport(), Models.soundcloudPlaylist.load())
        }, itunesReading:function ()
        {
            this.closeDialog();
            this.submitFunc = this.context = null;
            this.importing = !0;
            this.showDialog($("<div/>").attr("id", "dialog-import-itunes").addClass("dialog").css("left", Main.LEFT +
        ...
        }, itunesCreatePlaylist:function (a, b, c)
        {
            if (0 < b.length)
            {
                var d =
                    a;
                1 < c && (d += " " + c);
                if (200 >= b.length)Models.playlist.facade.create(d, b), this.iTunesImportNextPlaylist(); else
                {
                    var e = b.slice(0, 200);
                    Models.playlist.facade.create(d, e);
                    b.splice(0, 200);
        ...
        }, posDialogFields:function ()
        {
            ...
            if ($.browser.mozilla)$(".dialog-message").css("top",
                ...
                19); else if (Main.isChrome || Main.isSafari)$(".dialog-checkbox-container label").css("top", 1).css("left", 21), this.isRoomInfo && $(".dialog-menu-container label").css("left", 47).css("top", 4);
        ...
        }, getMenu:function (a, b, c, d, e)
        {
            ...
            for (var e = c.length, g = 0; g < e; ++g)c[g].hasOwnProperty("label") ? d == c[g].value ? f.append($('<option selected="selected" value="' + c[g].value + '">' +
                c[g].label + "</option>")) : f.append($('<option value="' + c[g].value + '">' + c[g].label + "</option>")) : d == c[g] ? f.append($('<option selected="selected">' + c[g] + "</option>")) : f.append($("<option>" + c[g] + "</option>"));
        ...
        {
            var a = Models.user.data.language || "en", b = $("<select/>").attr("id", "dialog-menu-language"), c = $("<optgroup/>").attr("label",
                Lang.dialog.userInterface), d = Models.chat.uiLanguages.length, e;
            ...
            for (e = 0; e < d; ++e)a == Models.chat.chatLanguages[e].value ?
                c.append($('<option selected="selected" value="' + Models.chat.chatLanguages[e].value + '">' + Models.chat.chatLanguages[e].label + "</option>")) : c.append($('<option value="' + Models.chat.chatLanguages[e].value + '">' + Models.chat.chatLanguages[e].label + "</option>"));
            b.append(c);
        ...
        {
            var b;
            if (window.event)b = window.event.keyCode; else if (a)b = a.which; else return!0;
            ...
            {
                if (27 == b || 0 == b)return a.stopPropagation(), a.preventDefault(), Dialog.forceCallback && null != Dialog.context && Dialog.context(), Dialog.closeDialog(), !1;
        ...
        }, checkAvailability:function ()
        {
            ...
            if (a != this.lastUsername)if (this.lastUsername = a, a == Models.user.data.username)$("#username-availability").text(Lang.dialog.displayNameAvailable).css("color", "#42a5dc"),
                this.nameAvailable = !0; else if (1 < a.length)if (-1 == a.indexOf("http://"))$("#username-availability").text(""), this.showAvailabilityProgress(), (new UserDisplayNameAvailableService(a)).successCallback = $.proxy(this.onAvailabilityResponse, this); else this.onAvailabilityResponse(!1)
        ...
        {
            this.hideAvailabilityProgress();
            this.availabilityProgress = (new Spinner({lines:8, length:0, width:3, radius:4, color:"#fff"})).spin();
        ...
        }, show:function (a, b)
        {
            ...
            {
                var g = $("<span/>").text(c[f].name).addClass("pop-menu-row-label"), h = $("<li/>").data("item", c[f]).mousedown($.proxy(this.onMenuClick, this)).append(g);
        ...
        {
            this.context && (a = $("body").find(a.currentTarget).data("item"), this.target.data("curate") ? a ? this.curateItem(a) : Dialog.createPlaylist(null, !0) : a ? Models.playlistMedia(a.id).mediaInsert(this.context, -1, this.context.length) :
                Dialog.createPlaylist(this.context));
            this.hide()
        ...
        {
            this.hide()
        }}), Audience = Class.extend({init:function ()
    {
    ...
    }, initRoomElements:function ()
    {
        ...
        {
            this.roomElements = [
                {image:this.arch, hitArea:this.archHit, props:{x:46, y:102, w:177, h:110}},
                {image:this.mountain, hitArea:this.mountainHit, props:{x:871, y:73, w:345, h:163}},
                {image:this.cactus, hitArea:this.cactusHit, props:{x:988, y:160, w:113, h:125}},
                {image:this.cloud, hitArea:this.cloudHit, props:{x:870, y:4, w:121, h:171}}
            ]
        }, onMouseMove:function (a)
        {
            var b = a.pageX - this.offset.left, a = a.pageY - this.offset.top, c = this.imageMapContext.getImageData(b, a, 1, 1);
            if (255 == c.data[3])
            {
                var d;
                if (c =
                    this.colorMap[this.colorLookup[255 * c.data[1] + c.data[2]]])for (var e = c.length, f = 0; f < e; ++f)
                {
                    var g = c[f];
        ...
        {
            var a;
            this.isRendered ? this.update() : (a = this.render()) ? 12 > ++this.renderPass ? this.renderedFrames[this.frame] =
                this.renderMap ? {audience:this.context.getImageData(0, 0, 1230, 306), map:this.imageMapContext.getImageData(0, 0, 1230, 306)} : {audience:this.context.getImageData(0, 0, 1230, 306)} : this.isRendered = !0 : this.refresh()
        }, update:function ()
        {
        ...
        }, renderClip:function ()
        {
            ...
            for (var a = this.images.length, b = !0, c = 0; c < a; ++c)
            {
                ...
                } else this.context.drawImage(d.image, d.props.x, d.props.y, d.props.w, d.props.h), this.imageMapContext.drawImage(d.hitArea, d.props.x, d.props.y, d.props.w, d.props.h); else b = !1
            }
            return b
        ...
        {
            for (var b =
                a._position.r, c = a._position.c, d = (1 - this.gridData.backScale) / this.gridData.rows, e = a.id != Models.user.data.id ? 114 : 117, f = 0.5, g = this.gridData.cellSize * f, h = 0; h < b; ++h)f = d * h + this.gridData.backScale, g = this.gridData.cellSize * f, h < b && (e += g);
            d = 15 + (1200 - this.gridData.cellSize * this.gridData.columns * f) / 2;
            b = this.avatarColors[a.avatarID] || 1;
            c = this.userImageMap[a.id] = {user:a, avatarID:a.avatarID, color:b, props:{x:~~(d + c * g + g / 2 - Avatars.audience.w / 2 * f), y:~~(e + g / 2 - Avatars.audience.h * f), w:Avatars.audience.w * f, h:Avatars.audience.h *
                f, sw:Avatars.audience.sw * f, scale:f, ox:d + c * g + g / 2 - Avatars.audience.w / 2, oy:e + g / 2 - Avatars.audience.h, ow:Avatars.audience.w, oh:Avatars.audience.h}};
            c.props.rect = {l:Math.round(c.props.x + 35 * c.props.scale), t:Math.round(c.props.y + 50 * c.props.scale), r:Math.round(c.props.x + c.props.w - 35 * c.props.scale), b:Math.round(c.props.y + c.props.h - 5 * c.props.scale)};
            this.colorMap["" + b] || (this.colorMap["" + b] = []);
            this.colorMap["" + b].push(c);
            this.avatarImages[a.avatarID] ? (c.image = this.avatarImages[a.avatarID], this.loadHitArea(c)) :
                this.loadAvatar(c);
        ...
        }, updateUser:function (a)
        {
            if (this.userImageMap[a.id])
            {
                ...
                a.avatarID =
                    b.avatarID;
                a.props = {x:~~(c + d * h + h / 2 - Avatars.audience.w / 2 * g), y:~~(f + h / 2 - Avatars.audience.h * g), w:Avatars.audience.w * g, h:Avatars.audience.h * g, sw:Avatars.audience.sw * g, scale:g, ox:c + d * h + h / 2 - Avatars.audience.w / 2, oy:f + h / 2 - Avatars.audience.h, ow:Avatars.audience.w, oh:Avatars.audience.h};
                a.props.rect = {l:Math.round(a.props.x + 35 * a.props.scale), t:Math.round(a.props.y + 35 * a.props.scale), r:Math.round(a.props.x + a.props.w - 35 * a.props.scale), b:Math.round(a.props.y + a.props.h - 5 * a.props.scale)};
                this.avatarImages[b.avatarID] ?
                    (a.image = this.avatarImages[b.avatarID], this.loadHitArea(a, !0)) : this.loadAvatar(a, !0)
            }
        }, sortZIndex:function ()
        {
            var a = this.mountain, b = this.arch;
            this.images.sort(function (c, d)
            {
                var e = ~~(c.props.y + c.props.h), f = ~~(d.props.y + d.props.h);
                if (c.user && d.user)
                {
                ...
                } else if (c.image == a || c.image == b)e -= 15; else if (d.image == a || d.image == b)f -= 15;
                return e > f ? 1 :
                    e < f + f ? -1 : 0
            });
            ...
            for (var h in this.colorMap)this.colorMap[h].sort(function (a, b)
            {
                return a.props.zindex > b.props.zindex ?
                    1 : a.props.zindex < b.props.zindex ? -1 : 0
            })
        ...
        }, onHitAreaLoaded:function (a, b, c)
        {
            b.hitArea = a;
            if (c)this.refresh(),
                this.draw(); else this.onImageLoaded()
        ...
        }}), DJBooth = Class.extend({init:function ()
    {
    ...
    {
        var c, d = 0;
        0 < b ? (c = Avatars.booth, d = 77 * (5 - b - 1)) :
            c = Avatars.dj;
        c = this.userImageMap[a.id] = {user:a, avatarID:a.avatarID, slot:b, props:{x:d, y:0, w:c.w, h:c.h, scale:1, sw:c.sw, ox:d, oy:0, ow:c.w, oh:c.h}};
    ...
    }, updateUser:function (a)
    {
        if (this.userImageMap[a.id])
        {
            var b, c = 0, d = this.userImageMap[a.id];
            0 < d.slot ? (b = Avatars.booth, c = 77 * (5 - d.slot - 1)) : b = Avatars.dj;
            d.avatarID = a.avatarID;
            d.props = {x:c, y:0, w:b.w, h:b.h, scale:1, sw:b.sw, ox:c, oy:0, ow:b.w, oh:b.h};
    ...
    {
        if (this.isRendered)this.update(); else if (this.render())
        {
            if (!Main.isMac || !Main.isSafari)12 > ++this.renderPass ? this.renderedFrames[this.frame] = {booth:this.bContext.getImageData(0, 0, 375, 205), dj:this.djContext.getImageData(0, 0, 170, 220)} : this.isRendered = !0
        } else this.refresh()
    }, update:function ()
    {
        this.renderedFrames[this.frame].dj && this.djContext.putImageData(this.renderedFrames[this.frame].dj, 0, 0);
        this.renderedFrames[this.frame].booth &&
        this.bContext.putImageData(this.renderedFrames[this.frame].booth, 0, 0)
    }, setRenderMode:function (a)
    {
        this.render = a ? this.renderClip : this.renderNoClip
    }, renderClip:function ()
    {
        ...
        for (var a = !0, b = this.images.length; b--;)
        {
            ...
            {
                var d = 1 == c.user.vote || 0 == b ? this.frame : 0, e = 0 < b ? this.bContext : this.djContext;
                e.save();
                e.beginPath();
    ...
    }, renderNoClip:function ()
    {
        ...
        for (var a = !0, b = this.images.length; b--;)
        {
            ...
            if (c.image && c.image.complete)
            {
                var d = 1 == c.user.vote || 0 == b ? this.frame : 0, e = 0 < b ? this.bContext :
                    this.djContext;
                e.save();
        ...
        return a
    }}), SearchView = Class.extend({init:function ()
    {
        this.data = [];
        this.format = "1";
        ...
        this.suggestIndex = -1;
        this.suggestions = [];
        this.suggestSelecting = this.submitPending =
            !1;
    ...
    }, onKeyUp:function (a, b)
    {
        ...
        if ("0" != this.format)
        {
            ...
            if (40 == c || 38 == c)
            {
                this.suggestSelecting =
                    !0;
                40 == c ? ++this.suggestIndex : --this.suggestIndex;
                -2 == this.suggestIndex ? this.suggestIndex = this.suggestions.length - 1 : this.suggestIndex == this.suggestions.length && (this.suggestIndex = -1);
            ...
            if (27 == c || 0 == c)this.query = a.value = "", this.onSuggestUpdate(null), clearTimeout(this.suggestTimeoutID), $("#button-clear-search").hide(); else
            {
                if (13 == c && "" != a.value)b.preventDefault(), this.onSubmitSearch(b); else
                {
                    this.query = a.value;
                    -1 < this.suggestTimeoutID && clearTimeout(this.suggestTimeoutID);
                    if ("" != a.value)this.suggestTimeoutID = setTimeout($.proxy(this.onSuggestTimeout,
                        this), 100); else this.onSuggestUpdate([]);
                    0 == a.value.length ? $("#button-clear-search").hide() : $("#button-clear-search").show()
                }
                return!1
            }
        ...
        {
            if (27 == c || 0 == c)this.query = a.value = "", clearTimeout(this.suggestTimeoutID), $("#button-clear-search").hide(); else return 13 == c && "" != a.value ? (b.preventDefault(), this.format = "1", this.updateSearchPlaceholder(), this.onSubmitSearch(b)) : (this.query = a.value, 0 == a.value.length ? $("#button-clear-search").hide() : $("#button-clear-search").show(), MediaOverlay.onSearchLocal(this.query.toLowerCase())),
                !1;
    ...
    }, onSearchPlaylistsClick:function ()
    {
        if ("0" == this.format)this.format =
            "1"; else if (this.format = "0", 0 < this.query.length)MediaOverlay.onSearchLocal(this.query);
        this.updateSearchPlaceholder()
    ...
    {
        clearTimeout(this.suggestTimeoutID);
        if (0 < this.query.length)this.ytSuggestService.load(this.query); else this.onSuggestUpdate([])
    ...
    {
        var b = this.suggestIndex =
            $(a.currentTarget).data("index");
        $(".search-suggestion-item").each(function (a)
        {
    ...
    {
        if (window.File &&
            window.FileReader && window.FileList && window.Blob)if (a = a.target.files, 0 < a.length)
        {
        ...
        } else _gaq.push(["_trackEvent", "Actions", "Import", "Turntable : Cancel"]); else alert("This browser does not support the feature required to import from tt.")
    }}), ChatView = Class.extend({init:function ()
    {
    ...
    }, onKeyPress:function (a, b)
    {
        var c;
        if (window.event)c =
            window.event.keyCode; else if (b)c = b.which; else return!0;
    ...
    }, onKeyUp:function (a, b)
    {
        ...
        if (0 <
            d)
        {
            var e = this.chatInput.value.substr(0, d), f = e.lastIndexOf(" @");
            -1 == f && (f = e.indexOf("@"));
            -1 < f && (0 == f ? this.suggestions = Models.room.mentionLookup(e.substr(f + 1, d)) : 0 < f && d > f + 2 && (this.suggestions = Models.room.mentionLookup(e.substr(f + 2, d))))
        }
        if (9 == c && 0 < this.suggestions.length)return b.preventDefault(), b.stopImmediatePropagation(), !1;
        40 == c || 38 == c ? (40 == c ? ++this.suggestIndex : --this.suggestIndex, -1 == this.suggestIndex ? this.suggestIndex = this.suggestions.length - 1 : this.suggestIndex == this.suggestions.length && (this.suggestIndex =
            0), this.updateSelectedSuggestion()) : this.updateSuggestions();
        return!0
    }, onChatReceived:function (a)
    {
        var a = a.message, b = $("<div/>").addClass("chat-" + a.type);
        if (a.from)
        {
            ...
            a.from.admin ? b.addClass("chat-admin") : a.from.ambassador ? b.addClass("chat-ambassador") : a.from.owner ? b.addClass("chat-host") : a.from.staff && (4 == a.from.staff ? b.addClass("chat-cohost") :
                3 == a.from.staff ? b.addClass("chat-manager") : 2 == a.from.staff && b.addClass("chat-bouncer"));
        ...
        a.lang &&
        b.append($("<span/>").addClass("chat-language").html(a.lang.html).attr("title", a.lang.title));
    ...
    }, onTimestampUpdate:function (a)
    {
        ...
        {
            var b = $(this), c = b.text(), d = -1 < c.indexOf("am") || -1 < c.indexOf("pm");
            if ("24" == a.value && d)d = c.split(":")[0], -1 < c.indexOf("pm") && (d = parseInt(d) +
                12), b.text(d + ":" + c.substr(0, c.length - 2).split(":")[1]); else if ("12" == a.value && !d)
            {
                var d = parseInt(c.split(":")[0]), e = "am";
                12 < d && (e = "pm", d -= 12);
    ...
    }, onFromClick:function (a)
    {
        ...
        if (b)if (Popout)Popout.Chat.onFromClick(a, b); else
        {
            var c = Models.room.userHash[b] || Models.room.nirUserHash[b];
            if (c)this.onShowChatUser(c, {x:$("#chat").offset().left, y:$(a.currentTarget).offset().top}); else this.getExternalUser(b, {x:$("#chat").offset().left, y:$(a.currentTarget).offset().top},
    ...
    }, onDeleteChatClick:function (a)
    {
        ...
        {
            Popout = window.open("/_/popout/", "chatpopout", "width=348,height=424,location=no,menubar=no,titlebar=no,status=no,toolbar=no,resizable=yes,left=" +
                (screen.width - 348) / 2 + ",top=" + (screen.height - 424) / 2);
            $("#chat").hide();
        ...
        }, getMentionRange:function ()
        {
            ...
            if (0 < a)
            {
                var b = this.chatInput.value.substr(0, a), c = b.lastIndexOf(" @");
                -1 == c ? c = b.indexOf("@") : ++c;
                return[c + 1, a]
            }
        ...
        }, updateSuggestions:function ()
        {
            ...
            if (0 == a)this.suggestionContainer.hide(), this.suggestIndex = -1; else
            {
                for (var b = 0; b < a; ++b)
                {
        ...
        {
            var a = this.getMentionRange(), b = this.chatInput.value.substr(0, a[0]), c = this.chatInput.value.substr(a[1]);
...
swfobject.embedSWF("http://plug.dj/_/static/swf/sfx.dea53bdb.swf", "chat-sound", "1", "1", "10", null, null, {quality:"high", wmode:"opaque", allowScriptAccess:"always", allowFullScreen:"false", bgcolor:"#000000"}, {id:"chat-sound"});
var Main, Slug, Playback, Chat, Room, RoomUser, TransactionManager, Utils, Models, MediaOverlay, UserListOverlay, AvatarOverlay, Avatars, Dialog, PopMenu, Search, rpcGW, DB, LS, Lobby, API, SocketListener, SC, WML, Popout, isiOS = navigator.userAgent.match(/(iPad|iPhone|iPod)/i) ? !0 : !1, isAndroid = -1 < navigator.userAgent.indexOf("android"), EXT = EXT || {}, MainView = Class.extend({init:function ()
{
    this.WIDTH = 1200;
    this.LEFT = 0;
    DB = new LSDatabase;
    ...
    a.mozilla && $("html").keypress(function (a)
    {
        var c = $("*:focus")[0];
        return!c || c.nodeName != "INPUT" ? a.which !== 0 : true
    });
...
function log(a)
{
    Models.chat.receive({type:"update", message:a})
}
